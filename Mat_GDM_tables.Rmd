...
diabetes_table <- df %>%
  ...
  summarise(
    ...
    # GDM CATEGORIES:
    "GDM = Yes " = 
      paste0(sum(GEST_DIAB_MEAS == 1),
             " (",
             format(round(sum(GEST_DIAB_MEAS == 1)/n()*100,2), nsmall=2, digits=3),
             ")"),
    
    "GDM = No " = 
      paste0(sum(GEST_DIAB_MEAS == 0),
             " (",
             format(round(sum(GEST_DIAB_MEAS == 0)/n()*100,2), nsmall=2, digits=3),
             ")")) %>%
