...
```{r}
merged_df <- merged_df %>%
  mutate(DELIVERED_FF = case_when((!is.na(M09_DELIV_DSSTDAT_INF1_6) | M09_DELIV_DSSTDAT_INF1_6!="1907-07-07") ~ 1,
                                  TRUE ~ as.numeric(0)))
...
# ADD UNSCHEDULED VISITS - CURRENTLY, I ONLY HAVE ONE.
...
merged_df <- merged_df %>%
  rowwise() %>%
  mutate(GEST_HYPER_MEAS = ifelse(((sum(is_ghtn(meanSBP_ANC20, meanDBP_ANC20),
...
table(merged_df$GEST_HYPER_MEAS, merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$GEST_HYPER_MEAS, merged_df$SITE, useNA = "always"), margin=2)*100,2)
...
merged_df <- merged_df %>% 
  mutate(SEVERE_GEST_HYPER_MEAS = ifelse(((sum(is_severe_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                         ...
                                         is_severe_ghtn(meanSBP_Hosp_ANC, meanDBP_Hosp_ANC), na.rm = TRUE) >=2) &
                                      CHRON_HYPER==0), 1, 0))
...
######################################
merged_df <- merged_df %>%                      # VISIT 20
  mutate(GEST_HYPER_TREAT = case_when((1 %in% c(M04_HPD_OECTRT_3_2, M04_HPD_OECTRT_4_2, M04_HPD_OECTRT_5_2, # ANC
...
#TODO - NEED TO FIX GESTATIONAL HYPERTENSION VARIBLE TO INCLUDE ANY 2 TP AND TO INLCUDE MEDICATION USE.
##################################################################################
merged_df <- merged_df %>%
  mutate(MAT_PE_SEV_MEAS = case_when((SEVERE_GEST_HYPER_MEAS==1 |
                                           ...
                                           M08_CREAT_UMOLL_LBORRES_4 > 1.1) ~ 1, # KIDNEY FUNCTION 
                                         TRUE ~ as.numeric(0)))
