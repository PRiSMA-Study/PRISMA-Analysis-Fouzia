...
README: 
As of 04/01/2024 This is the first file that needs to be run before doing constructed outcomes!
...
This file is created from PRISMA_Constructed-Outcomes_IndivForms(v1) and (v2).Rmd.  This is the active file as of 10/04/2023 
...
# path_to_data <- 'Z:/Stacked Data/2023-10-13/' # - for AWS data

path_to_data <-  'D:/Users/fouziafarooq/Documents/PRISMA-Analysis-Fouzia/data/2024-06-28/'# - for AWS data - stacked. 
...
mnh_list <- readRDS(file = 'MNH_list_2024-06-28.RDS')
```   
...
```{r echo=FALSE}
###############################
 # Only need to run this line outside of AWS when I need to add Mxx prefix.
################################
...
merged_df <- merged_df %>% 
  mutate(ENROLLED_FF = case_when(M02_AGE_IEORRES_1 == 1 & M02_PC_IEORRES_1 == 1 & M02_CATCHMENT_IEORRES_1 == 1 &
                       M02_CATCH_REMAIN_IEORRES_1 == 1  & (M02_SCRN_RETURN_1 != 0 | is.na(M02_SCRN_RETURN_1)) ~ 1,
...
CONSENT = case_when(!is.na(M02_CONSENT_IEORRES_1) ~ M02_CONSENT_IEORRES_1,
                    TRUE ~ 99),
ENROLL = case_when(M02_CONSENT_IEORRES_1 == 1 & !is.na(M02_FORMCOMPLDAT_MNH02_1) & ENROLLED_FF == 1 ~ 1,## MAY7 UPDATES: (M02_SCRN_RETURN == 1 | is.na(M02_SCRN_RETURN) to account for particpants who returned for screening and/or sites not using the variable
                   ENROLLED_FF == 0 | M02_CONSENT_IEORRES_1 == 0 | M02_CONSENT_IEORRES_1 == 77 ~ 0,
                   TRUE ~ 77))
  
table(merged_df$ENROLLED_FF, useNA = "always")

merged_df <- merged_df %>%
  filter(ENROLLED_FF==1)
...
# ANC 4 NOT FILLED OUT AT DELIVERY (VISIT 6)
```{r}
...
# MNH08 IS OPTIONAL AT DELIVERY - VISIT 6(SO SOME PLACES WILL HAVE, SOME WONT.) - NOT INCLUDING HERE ATM.
```{r}
...
# SETTING UP: IS MNH06 VISIT COMPLETE? 
# MNH06 is optional at delivery
```{r}
mnh06_constructed_df <- mnh06_df %>% 
  rowwise() %>%
  # VISIT 1
  mutate(M06_MAT_VISIT_FORM_COMPLETED_1 = case_when(!is.na(M06_MAT_VISIT_MNH06_1) ~ 1, # M06_MAT_VISIT_MNH06_1 is always 1,2, 3 etc number or NA
                                                    TRUE ~ as.numeric(0)),
         M06_MAT_VISIT_wPERSON_COMPLETED_1 = case_when(M06_MAT_VISIT_MNH06_1 %in% c(1,2) ~ 1, 
                                                       TRUE ~ 0),
         # VISIT 2
         M06_MAT_VISIT_FORM_COMPLETED_2 = case_when(!is.na(M06_MAT_VISIT_MNH06_2) ~ 1, 
                                                    TRUE ~ 0),
         M06_MAT_VISIT_wPERSON_COMPLETED_2 = case_when(M06_MAT_VISIT_MNH06_2 %in% c(1,2) ~ 1, 
                                                       TRUE ~ 0),
         # VISIT 3
         M06_MAT_VISIT_FORM_COMPLETED_3 = case_when(!is.na(M06_MAT_VISIT_MNH06_3) ~ 1, 
                                                    TRUE ~ 0),
         M06_MAT_VISIT_wPERSON_COMPLETED_3 = case_when(M06_MAT_VISIT_MNH06_3 %in% c(1,2) ~ 1, 
                                                       TRUE ~ 0),
         # VISIT 4
         M06_MAT_VISIT_FORM_COMPLETED_4 = case_when(!is.na(M06_MAT_VISIT_MNH06_4) ~ 1, 
                                                    TRUE ~ 0),
         M06_MAT_VISIT_wPERSON_COMPLETED_4 = case_when(M06_MAT_VISIT_MNH06_4 %in% c(1,2) ~ 1, 
                                                       TRUE ~ 0),
         # VISIT 5
         M06_MAT_VISIT_FORM_COMPLETED_5 = case_when(!is.na(M06_MAT_VISIT_MNH06_5) ~ 1, 
                                                    TRUE ~ 0),
         M06_MAT_VISIT_wPERSON_COMPLETED_5 = case_when(M06_MAT_VISIT_MNH06_5 %in% c(1,2) ~ 1, 
                                                       TRUE ~ 0))
          # VISIT 13 Hosp
        # M06_MAT_VISIT_FORM_COMPLETED_13 = case_when(!is.na(M06_MAT_VISIT_MNH06_13) ~ 1, 
         #                                           TRUE ~ 0),
        # M06_MAT_VISIT_wPERSON_COMPLETED_13 = case_when(M06_MAT_VISIT_MNH06_13 %in% c(1,2) ~ 1, 
         #                                              TRUE ~ 0))

# EVERYONE AT VISIT 1 HAS MNH06 IN PAKISTAN:
  test.df <- mnh06_constructed_df %>%
    select(MOMID, PREGID, SITE, M06_DIAG_VSDAT_1, M06_MAT_VISIT_MNH06_1, M06_MAT_VISIT_FORM_COMPLETED_1) # %>% filter(SITE=="Kenya")
  
  # NEED THIS CONSTRUCTED_DF2 TO BE MERGED NEXT:
  mnh06_constructed_df2 <- mnh06_constructed_df %>%
    select(MOMID, PREGID, SITE, starts_with(c( "M06_MAT_VISIT_FORM_COMPLETED_", "M06_MAT_VISIT_wPERSON_COMPLETED_")))
```
...
merged_df <- left_join(merged_df, mnh06_constructed_df2,
                                        by = c("MOMID", "PREGID", "SITE"))

# THIS WILL HAVE NA'S FOR COMPLETED FORM IF A SITE DOESN'T HAVE ANY MNH06 SO WHEN IT MERGED BACK INTO MY MERGED_DF, IT GET'S MERGED AS NA.
test.df <- merged_df %>% 
  select(MOMID, PREGID, SITE, M06_DIAG_VSDAT_1, M06_MAT_VISIT_MNH06_2, M06_MAT_VISIT_FORM_COMPLETED_2) # %>% filter(SITE=="Kenya")
...
temp.df <- merged_df %>% 
  ...
  select(SITE, MOMID, PREGID, M09_HDP_HTN_MHOCCUR_1_6, M09_HDP_HTN_MHOCCUR_77_6, M09_HDP_HTN_MHOCCUR_99_6, M09_PREECLAMPSIA_CEOCCUR_1_6, M09_PREECLAMPSIA_CEOCCUR_77_6, M09_PREECLAMPSIA_CEOCCUR_99_6)
```

```{r}
## visit windows for ANC
merged_df <- merged_df %>% 
  # filter(M01_US_OHOSTDAT_1 != ymd("1907-07-07")) %>%   
  ...
  mutate(ENROLL_ONTIME = (EDD_BOE - as.difftime(280, unit="days")) + as.difftime(139, unit="days"), #EDD-280 = conception date + and then estimate the right hand of the window. Gives a date
         ...
         ANC20_ONTIME = (EDD_BOE - as.difftime(280, unit="days")) + as.difftime(160, unit="days"), # computing date for 20 weeks
         ANC20_LATE = (EDD_BOE - as.difftime(280, unit="days")) + as.difftime(181, unit="days"), # she has 21 days for ANC20 and then she's late for that visit. 
  ...
  mutate(ENROLL_PASS = ifelse(ENROLL_ONTIME<UploadDate, 1, 0),  # WINDOW HAS PASSED. IF YOUR ENROLL WINDOW END IS JAN 1ST, BUT UPLOAD JAN 5TH. THEN YOU'RE LATE MEANING ON TIME WINDOW IS PASSED. 
         ANC20_PASS = ifelse(ANC20_ONTIME<UploadDate, 1, 0), #IF A WINDOW IS PASSED AND HER GA AT ENROLLEMENT WAS <17 WEEKS, SHE DOES GET AN ANC 20. IF SHE'S >18 THEN HER ANC20 FORMS WILL BE NA B/C SHE'S NOT SUPPOSED TO HAVE IT.
          ANC20_PASS = ifelse(ANC20_ONTIME<UploadDate & GA_ENROLL_WKS <= 17, 1, 0), # NEED TO ASK STACY WHY SHE DOESN'T HAVE THIS LINE INSTEAD. (01/31/2024)
  ...
  ## CALCULATE INDICATOR VARIABLES for passed LATE ANC WINDOWS
  mutate(ENROLL_PASS_LATE = ifelse(ENROLL_LATE<UploadDate, 1, 0),
         ANC20_PASS_LATE = ifelse(ANC20_LATE<UploadDate & GA_ENROLL_WKS <= 17, 1, 0), # It said BOE_GA_WKS_ENROLL when i ran this on 04/01/2024 (why?)
...
merged_df <- merged_df %>%
  ...
  mutate(M04_ENROLL_OVERDUE = ifelse(UploadDate>ENROLL_ONTIME & (M04_MAT_VISIT_FORM_COMPLETED_1==0), 1, 0),
         M08_ENROLL_OVERDUE = ifelse(UploadDate>ENROLL_ONTIME & (M08_MAT_VISIT_FORM_COMPLETED_1==0), 1, 0),#TODO DO THIS FOR MNH08
          ...
          # if this window ANC20 is passed, then this var would be NA b/c they don't have to have ANC20 visit form. 
...
temp.df <- merged_df %>% select(MOMID, SITE, GA_AT_DELIV_6, DOB_DATEONLY, ANC36_ONTIME, ANC36_PASS, 
                                ANC36_PASS_LATE, M04_MAT_VISIT_FORM_COMPLETED_5, M04_ANC_OBSSTDAT_5) %>% 
  
  mutate(ANC36_PASS_FF = case_when(is.na(DOB_DATEONLY) & (ANC36_ONTIME<UploadDate) ~ 1,
                                   !is.na(DOB_DATEONLY) & (DOB_DATEONLY>=ANC36_ONTIME) ~ 1,
                                   TRUE ~ 0))
```



# GA INDICATOR VARIABLES
# When her GA at enrollment is <=17, then M04_GA_ENROLL_LT18==1 and she should have an ANC20 (VISIT 2) form then.
# INVESTIGATING GA AT VISIT AND DELIVERY GA
```{r}
merged_df <- merged_df %>%
  mutate(M04_GA_ENROLL_LT18 = case_when((M04_GA_AT_VISIT_1/7) <=17 ~ 1,
                                        TRUE ~ as.numeric(0)))


temp.df <- merged_df %>%
  select(MOMID, SITE, M04_GA_AT_VISIT_1, M04_GA_AT_VISIT_2, M04_GA_AT_VISIT_3, 
         M04_GA_AT_VISIT_4, M04_GA_AT_VISIT_5, ANC28_PASS, ANC32_PASS,ANC32_PASS_LATE, ANC36_PASS, ANC36_PASS_LATE,
         M04_MAT_VISIT_FORM_COMPLETED_4, M04_MAT_VISIT_FORM_COMPLETED_5, GA_AT_DELIV_6) %>%
  mutate(GA_VISIT1 = as.double(M04_GA_AT_VISIT_1)/7,
         GA_VISIT2 = as.double(M04_GA_AT_VISIT_2)/7,
         GA_VISIT3 = as.double(M04_GA_AT_VISIT_3)/7,
         GA_VISIT4 = as.double(M04_GA_AT_VISIT_4)/7, 
         GA_VISIT5 = as.double(M04_GA_AT_VISIT_5)/7,
         GA_DELIV = as.double(GA_AT_DELIV_6)/7) %>% 
  select(-M04_GA_AT_VISIT_1, -M04_GA_AT_VISIT_2, -M04_GA_AT_VISIT_3,-M04_GA_AT_VISIT_4, -M04_GA_AT_VISIT_5, -GA_AT_DELIV_6)
```

# WRITE OUT A MERGED FILE RDS
```{r}
write_rds(merged_df, '../data_out/merged_df_20240628.RDS')
```
...
write.csv(window_subset_df, '../data_out/window_subset_df_20240628.csv')
```
...
diabetes_df <- merged_df %>%
  select(MOMID, PREGID, SITE, DELIVERED_FF, M08_LBSTDAT_1, M08_LBSTDAT_3,
        ...
        #MEASURED VARS:
        M08_HBA1C_LBORRES_1, # for ghana only (hba1c)
        M08_HBA1C_PRCNT_1, # for all other sites (hba1c)
        M08_BGLUC_PRETEST_MMOLL_LBORRES_3, 
        M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_3, M08_BGLUC_ORAL_2HR_MMOLL_LBORRES_3,
        
        # Visit 4
        M08_BGLUC_PRETEST_MMOLL_LBORRES_4, M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_4, M08_BGLUC_ORAL_2HR_MMOLL_LBORRES_4,
        
        # visit 5: 
        M08_BGLUC_PRETEST_MMOLL_LBORRES_5, M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_5, M08_BGLUC_ORAL_2HR_MMOLL_LBORRES_5)

write.csv(diabetes_df, '../data_out/mat_diabetes_vars_20240315.csv')
```

# WRITE OUT A FILE FOR SPHB ANALYSIS
```{r}
sphb_df <- merged_df %>% select(MOMID, PREGID, SITE, 
                                M06_SPHB_VSSTAT_1, M06_SPHB_LBORRES_1,
                                M06_SPHB_VSSTAT_2, M06_SPHB_LBORRES_2,
                                M06_SPHB_VSSTAT_3, M06_SPHB_LBORRES_3,
                                M06_SPHB_VSSTAT_4, M06_SPHB_LBORRES_4, # NOTE that Sphb is not done at week32 except Pakistan 
                                M06_SPHB_VSSTAT_5, M06_SPHB_LBORRES_5,
                                M06_SPHB_VSSTAT_10, M06_SPHB_LBORRES_10, # Week 6 PNC (Visit 10)
                                
                                M08_CBC_LBTSTDAT_1, M08_CBC_HB_LBORRES_1,
                                M08_CBC_LBTSTDAT_2, M08_CBC_HB_LBORRES_2,
                                M08_CBC_LBTSTDAT_3, M08_CBC_HB_LBORRES_3,
                                M08_CBC_LBTSTDAT_4, M08_CBC_HB_LBORRES_4,
                                M08_CBC_LBTSTDAT_5, M08_CBC_HB_LBORRES_5,
                                M08_CBC_LBTSTDAT_10, M08_CBC_HB_LBORRES_10, # Week 6 PNC (Visit 10)
                                
                                M06_HB_POC_LBPERF_1, M06_HB_POC_LBORRES_1, 
                                M06_HB_POC_LBPERF_2, M06_HB_POC_LBORRES_2,
                                M06_HB_POC_LBPERF_3, M06_HB_POC_LBORRES_3,
                                M06_HB_POC_LBPERF_4, M06_HB_POC_LBORRES_4,
                                M06_HB_POC_LBPERF_5, M06_HB_POC_LBORRES_5,
                                M06_HB_POC_LBPERF_6, M06_HB_POC_LBORRES_10)


write.csv(sphb_df, '../data_out/mat_Sphb_20240628.csv')
```

```{r}
temp.df <- merged_df %>% select(MOMID, SITE, M08_HBA1C_PRCNT_1, M08_GA_AT_VISIT_3, M08_LBSTDAT_3) %>%
                                  filter(SITE=="Pakistan") %>% filter(M08_HBA1C_PRCNT_1>=6.5) %>% 
  mutate(GA_at_visit3_wks = as.double(M08_GA_AT_VISIT_3/7))
...
#####################################
#### WRITE OUT CSV FILE  FOR HTN, PE
####################################
```{r}
htn_df <- merged_df %>% 
  select(MOMID, PREGID, SITE, DELIVERED_FF, 
         ...
         starts_with(c("M04_HPD_MHOCCUR_", "M09_HDP_HTN_PROCCUR_", "M09_PREECLAMPSIA_CEOCCUR_")), 
         M09_HEADACHE_CEOCCUR_6, M09_HEADACHE_CEONGO_6,
         M09_CONFUSION_CEOCCUR_6, M09_CONFUSION_CEONGO_6,
         starts_with(c("M09_OTHR_CEOCCUR_")),
         # BP:
         M19_BP_SYS_VSORRES_13, M19_BP_DIA_VSORRES_13, M19_BP_SYS_VSORRES_14, M19_BP_DIA_VSORRES_14,
         # Other labs
         M06_HB_POC_LBORRES_4, M06_HB_POC_LBORRES_5,
         M08_IBILIRUBIN_LBORRES_4, M08_IBILIRUBIN_LBORRES_5, 
         M08_CBC_HB_LBORRES_4, M08_CBC_HB_LBORRES_5,
         ...
         M08_CREAT_UMOLL_LBORRES_4, M08_CREAT_UMOLL_LBORRES_5,
...
write.csv(htn_df, '../data_out/mat_HDP.csv')
...
```{r}
protein_df <- merged_df %>% select(MOMID, SITE, ENROLLED_FF, DELIVERED_FF, M08_LBSTDAT_3, M08_UA_PROT_LBORRES_3) %>%
  filter(SITE=="Kenya", DELIVERED_FF==1)
```
...
table(PREG_DEATH=merged_df$PREG_DEATH, SITE = merged_df$SITE, useNA = "always") # data is mostly 2=Alive or 4 = loss to follow up or NA. 
...
sum(merged_df$M08_CBC_HB_LBORRES_1 %in% c(-5, -7))

table(MAT_ANEMIA_MEAS1 = merged_df$MAT_ANEMIA_MEAS1, useNA = "always", SITE = merged_df$SITE)
round(prop.table(table(merged_df$MAT_ANEMIA_MEAS1, useNA = "always", merged_df$SITE), margin=2)*100,2) 

temp.df <- merged_df %>%
   select(MOMID, SITE, DELIVERED_FF, MAT_ANEMIA_MEAS1, M08_CBC_HB_LBORRES_1, M06_HB_POC_LBORRES_1) %>% filter(SITE=="Kenya")
...
# NA 
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(is.na(M08_CBC_HB_LBORRES_1)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M08_CBC_HB_LBORRES_1))/n()*100, 2), nsmall = 2, digits = 3))

# -5
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_1 %in% c(-5)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_1 %in% c(-5))/n()*100, 2), nsmall = 2, digits = 3))

# -7
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_1 %in% c(-7)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_1 %in% c(-7))/n()*100, 2), nsmall = 2, digits = 3))


# M06_HB_POC_LBORRES_1
# NA 
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(is.na(M06_HB_POC_LBORRES_1)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M06_HB_POC_LBORRES_1))/n()*100, 2), nsmall = 2, digits = 3))

# -5
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M06_HB_POC_LBORRES_1 %in% c(-5)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M06_HB_POC_LBORRES_1 %in% c(-5))/n()*100, 2), nsmall = 2, digits = 3))

# -7
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M06_HB_POC_LBORRES_1 %in% c(-7)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M06_HB_POC_LBORRES_1 %in% c(-7))/n()*100, 2), nsmall = 2, digits = 3))
...
table(MAT_ANEMIA_MEAS1 = temp.df$MAT_ANEMIA_MEAS1, useNA = "always", SITE = merged_df$SITE)
round(prop.table(table(temp.df$MAT_ANEMIA_MEAS1, useNA = "always", temp.df$SITE), margin=2)*100,2)

table(temp.df$M08_CBC_HB_LBORRES_1, useNA = "always")
table(temp.df$M06_HB_POC_LBORRES_1, useNA = "always")

varlist = c("M08_CBC_HB_LBORRES_1", "M06_HB_POC_LBORRES_1", "MAT_ANEMIA_MEAS1")
factorvarlist = c("MAT_ANEMIA_MEAS1")

CreateTableOne(data = temp.df, 
               vars = varlist, factorVars = factorvarlist, strata = "SITE",
               addOverall = TRUE)
...
temp.df <- merged_df %>%
  select(SITE, MOMID, PREGID, M08_CBC_HB_LBORRES_2, M06_HB_POC_LBORRES_2, MAT_ANEMIA_MEAS2)

table(merged_df$MAT_ANEMIA_MEAS2, useNA = "always", merged_df$SITE)
round(prop.table(table(merged_df$MAT_ANEMIA_MEAS2, useNA = "always", merged_df$SITE), margin=2)*100,2)  
...
# M08_CBC_HB_LBORRES_2
# NA 
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(is.na(M08_CBC_HB_LBORRES_2)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M08_CBC_HB_LBORRES_2))/n()*100, 2), nsmall = 2, digits = 3))

# -5
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_2 %in% c(-5)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_2 %in% c(-5))/n()*100, 2), nsmall = 2, digits = 3))

# -7
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_2 %in% c(-7)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_2 %in% c(-7))/n()*100, 2), nsmall = 2, digits = 3))


# M06_HB_POC_LBORRES_2
# NA 
missing_df.n <- merged_df %>% group_by(SITE) %>% summarise(sum(is.na(M06_HB_POC_LBORRES_1)))
missing_df.freq <- merged_df %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M06_HB_POC_LBORRES_1))/n()*100, 2), nsmall = 2, digits = 3))
...
anemia_df <- merged_df %>% 
  select(MOMID, PREGID, SITE, M09_DELIV_DSSTDAT_INF1_6, DELIVERED_FF, starts_with(c("M08_CBC_HB_LBORRES_", "M06_HB_POC_LBORRES_")))

write.csv(anemia_df, paste0('../data_out/', "mat_anemia_foranalysis" ,".csv"), row.names=FALSE)


table(anemia_df$DELIVERED_FF, SITE = anemia_df$SITE, useNA = "always")

anemia_df2 <- anemia_df %>% filter(DELIVERED_FF==1) # FILTER TO THOSE WITH A DELIVERY DATE 
...
#######
#  M08_CBC_HB_LBORRES_
missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(is.na(M08_CBC_HB_LBORRES_7)))
missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M08_CBC_HB_LBORRES_7))/n()*100, 2), nsmall = 2, digits = 3))

# M06_HB_POC_LBORRES_
missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(is.na(M06_HB_POC_LBORRES_9))) 
missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(is.na(M06_HB_POC_LBORRES_9))/n()*100, 2), nsmall = 2, digits = 3))

######
# -5 #
######
#  M08_CBC_HB_LBORRES_
missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_6 %in% c(-5)))
missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_6 %in% c(-5))/n()*100, 2), nsmall = 2, digits = 3))


# M06_HB_POC_LBORRES_
missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(M06_HB_POC_LBORRES_9 %in% c(-5)))
missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(M06_HB_POC_LBORRES_9 %in% c(-5))/n()*100, 2), nsmall = 2, digits = 3))

######
# -7 #
######
#  M08_CBC_HB_LBORRES_
missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(M08_CBC_HB_LBORRES_5 %in% c(-7)))
missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(M08_CBC_HB_LBORRES_5 %in% c(-7))/n()*100, 2), nsmall = 2, digits = 3))

# M06_HB_POC_LBORRES_
# missing_df.n <- anemia_df2 %>% group_by(SITE) %>% summarise(sum(M06_HB_POC_LBORRES_5 %in% c(-7)))
# missing_df.freq <- anemia_df2 %>% group_by(SITE) %>% summarise(format(round(sum(M06_HB_POC_LBORRES_ %in% c(-7))/n()*100, 2), nsmall = 2, digits = 3))


# write.csv(anemia_df2, paste0('data_out/', "mat_anemia" ,".csv"), row.names=FALSE)
```
...
table(MAT_DEATH = merged_df$MAT_DEATH, SITE = merged_df$SITE) # data is mostly 2=Alive or 4 = loss to follow up or NA.
round(prop.table(table(MAT_DEATH = merged_df$MAT_DEATH, SITE = merged_df$SITE), margin=2)*100, 2) # data is mostly 2=Alive or 4 = loss to follow up or NA. 
```
...
```{r}
merged_df %>% group_by(SITE) %>% summarize(n=n(), Protein_0 = sum(M08_UA_PROT_LBORRES_1==0, M08_UA_PROT_LBORRES_2==0,
                                                           M08_UA_PROT_LBORRES_3==0, M08_UA_PROT_LBORRES_4==0,
                                                           M08_UA_PROT_LBORRES_5==0, M08_UA_PROT_LBORRES_6==0,
                                                           na.rm = TRUE),
                                           
                                           Protein_1 = sum(M08_UA_PROT_LBORRES_1==1, M08_UA_PROT_LBORRES_2==1,
                                                           M08_UA_PROT_LBORRES_3==1, M08_UA_PROT_LBORRES_4==1,
                                                           M08_UA_PROT_LBORRES_5==1, M08_UA_PROT_LBORRES_6==1,
                                                           na.rm = TRUE),
                                           
                                           Protein_2 = sum(M08_UA_PROT_LBORRES_1==2, M08_UA_PROT_LBORRES_2==2,
                                                           M08_UA_PROT_LBORRES_3==2, M08_UA_PROT_LBORRES_4==2,
                                                           M08_UA_PROT_LBORRES_5==2, M08_UA_PROT_LBORRES_6==2,
                                                           na.rm = TRUE),
                                           
                                           Protein_3 = sum(M08_UA_PROT_LBORRES_1==3, M08_UA_PROT_LBORRES_2==3,
                                                           M08_UA_PROT_LBORRES_3==3, M08_UA_PROT_LBORRES_4==3,
                                                           M08_UA_PROT_LBORRES_5==3, M08_UA_PROT_LBORRES_6==3,
                                                           na.rm = TRUE),
                                           
                                           Protein_4 = sum(M08_UA_PROT_LBORRES_1==4, M08_UA_PROT_LBORRES_2==4,
                                                           M08_UA_PROT_LBORRES_3==4, M08_UA_PROT_LBORRES_4==4,
                                                           M08_UA_PROT_LBORRES_5==4, M08_UA_PROT_LBORRES_6==4,
                                                           na.rm = TRUE),
                                           
                                           Protein_77 = sum(M08_UA_PROT_LBORRES_1==77, M08_UA_PROT_LBORRES_2==77,
                                                           M08_UA_PROT_LBORRES_3==77, M08_UA_PROT_LBORRES_4==77,
                                                           M08_UA_PROT_LBORRES_5==77, M08_UA_PROT_LBORRES_6==77,
                                                           na.rm = TRUE),
                                           
                                           Protein_NA = sum(is.na(M08_UA_PROT_LBORRES_1), is.na(M08_UA_PROT_LBORRES_2),
                                                           is.na(M08_UA_PROT_LBORRES_3), is.na(M08_UA_PROT_LBORRES_4),
                                                           is.na(M08_UA_PROT_LBORRES_5), is.na(M08_UA_PROT_LBORRES_6)))
                                           

df_long_1 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_1)
df_long_2 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_2)
df_long_3 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_3)
df_long_4 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_4)
df_long_5 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_5)
df_long_6 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_6)
df_long_7 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_7)
df_long_8 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_8)
df_long_9 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_9)
df_long_10 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_10)
df_long_11 <- merged_df %>% select(SITE, x = M08_UA_PROT_LBORRES_11)

df_long <- rbind(df_long_1, df_long_2, df_long_3, df_long_4, 
                 df_long_5, df_long_6, df_long_7, df_long_8,
                 df_long_9, df_long_10, df_long_11)

table(df_long$x, df_long$SITE, useNA = "always")
...
```{r echo=FALSE}
mnh08_df <- mnh_list[["mnh08"]]
with(mnh08_df %>% filter(SITE=="Kenya"), table(M08_UA_PROT_LBORRES, M08_TYPE_VISIT, useNA = "always"))

# merged_df <- merged_df %>% 
#   mutate(PROTEINURIA2 = case_when((c('2', '3') %in% c(M08_UA_PROT_LBORRES_2, M08_UA_PROT_LBORRES_3, M08_UA_PROT_LBORRES_4, 
#                                                 M08_UA_PROT_LBORRES_5, M08_UA_PROT_LBORRES_6, M08_UA_PROT_LBORRES_7,
#                                                 M08_UA_PROT_LBORRES_8, M08_UA_PROT_LBORRES_9, M08_UA_PROT_LBORRES_10,
#                                                 M08_UA_PROT_LBORRES_13, M08_UA_PROT_LBORRES_14)) ~ 1,
#                                  TRUE ~ as.numeric(0)))
merged_df <- merged_df %>% 
  mutate(PROTEINURIA_ANC_IPC = ifelse((!is.na(M08_UA_PROT_LBORRES_2) & (M08_UA_PROT_LBORRES_2 == 1 | M08_UA_PROT_LBORRES_2==2)) | 
                                (!is.na(M08_UA_PROT_LBORRES_3) & (M08_UA_PROT_LBORRES_3 == 1 | M08_UA_PROT_LBORRES_3==2)) |
                                (!is.na(M08_UA_PROT_LBORRES_4) & (M08_UA_PROT_LBORRES_4 == 1 | M08_UA_PROT_LBORRES_4==2)) |
                                (!is.na(M08_UA_PROT_LBORRES_5) & (M08_UA_PROT_LBORRES_5 == 1 | M08_UA_PROT_LBORRES_5==2)) |
                                (!is.na(M08_UA_PROT_LBORRES_6) & (M08_UA_PROT_LBORRES_6 == 1 | M08_UA_PROT_LBORRES_6==2)) | 
                                (!is.na(M08_UA_PROT_LBORRES_13) & (M08_UA_PROT_LBORRES_13 == 1 | M08_UA_PROT_LBORRES_13==2)), 
         1, 0))

temp.df <- merged_df %>% 
  select(MOMID, PREGID, PROTEINURIA_ANC_IPC, M08_UA_PROT_LBORRES_2,
         M08_UA_PROT_LBORRES_3,M08_UA_PROT_LBORRES_4,
         M08_UA_PROT_LBORRES_5,M08_UA_PROT_LBORRES_6,
         M08_UA_PROT_LBORRES_7,M08_UA_PROT_LBORRES_8,
         M08_UA_PROT_LBORRES_9, M08_UA_PROT_LBORRES_10, 
         M08_UA_PROT_LBORRES_13, M08_UA_PROT_LBORRES_14)

table(PROTEINURIA_ANC_IPC = merged_df$PROTEINURIA_ANC_IPC, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(PROTEINURIA_ANC_IPC = merged_df$PROTEINURIA_ANC_IPC, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```

# PROTEINURIA AT PNC & HOSP.
PNC0,1,4,6
```{r}
merged_df <- merged_df %>% 
  mutate(PROTEINURIA_PNC = ifelse((!is.na(M08_UA_PROT_LBORRES_7) & 
                                         (M08_UA_PROT_LBORRES_7 == 2 | M08_UA_PROT_LBORRES_7==3)) |
                                        (!is.na(M08_UA_PROT_LBORRES_8) & 
                                           (M08_UA_PROT_LBORRES_8 == 2 | M08_UA_PROT_LBORRES_8==3)) |
                                        (!is.na(M08_UA_PROT_LBORRES_9) & 
                                           (M08_UA_PROT_LBORRES_9 == 2 | M08_UA_PROT_LBORRES_9==3)) |
                                        (!is.na(M08_UA_PROT_LBORRES_7) & 
                                           (M08_UA_PROT_LBORRES_10 == 2 | M08_UA_PROT_LBORRES_10==3)) |
                                        (!is.na(M08_UA_PROT_LBORRES_14) & 
                                           (M08_UA_PROT_LBORRES_14 == 2 | M08_UA_PROT_LBORRES_14==3)), # HOSP.
                                      1,0))

table(PROTEINURIA_PNC = merged_df$PROTEINURIA_PNC, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(PROTEINURIA_PNC = merged_df$PROTEINURIA_PNC, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```
...
merged_df <- merged_df %>% 
  mutate(CHRON_HYPER_MEAS = case_when((meanSBP_ANC_LT20 >=140 | meanDBP_ANC_LT20 >=90)  ~ 1, 
                                     TRUE  ~ as.numeric(0)))
table(merged_df$CHRON_HYPER_MEAS, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$CHRON_HYPER_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

merged_df <- merged_df %>% 
  mutate(CHRON_HYPER_MEAS_NA = case_when((meanSBP_ANC_LT20 >=140 | meanDBP_ANC_LT20 >=90)  ~ 1, 
                                    (is.na(meanSBP_ANC_LT20) & is.na(meanDBP_ANC_LT20)) ~ as.numeric(NA),
                                     TRUE  ~ as.numeric(0)))

table(merged_df$CHRON_HYPER_MEAS_NA, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$CHRON_HYPER_MEAS_NA, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# CHRONIC HYPERTENSION MEDICATION USE/TREATMENT
```{r}
merged_df <- merged_df %>%
...
table(merged_df$CHRON_HYPER_TREAT, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$CHRON_HYPER_TREAT, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```
...
table(merged_df$CHRON_HYPER_DIAG, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$CHRON_HYPER_DIAG, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```
# CHRONIC HYPERTENSION MEASURED + TREATMENT + DIAG (FINAL)
```{r}

merged_df <- merged_df %>% 
  mutate(CHRON_HYPER = case_when((CHRON_HYPER_MEAS ==1 | CHRON_HYPER_TREAT==1 | 
                                               CHRON_HYPER_DIAG==1) ~ 1,
                                            TRUE ~ as.numeric(0)))
...
# NEED TO REVISE GESTATIONAL HYPERTENSION TO INCLUDE MEDICATION USE
...
#TODO Will have to recreate this variable using gestational age at some point (b/c women who are at GA 18 or 19 can still come in for ANC20 visit (VISIT 2) so to truly count this as 20week GA at least for gHTN - I need to look at GA (10/02/2023)
...
# WHY DIDN'T WE INCLUDE LACK OF PROTEINURIA IN gHTN?
###################################################################################################
...
# gHTN MEASURED ANY 2 TIMEPOINTS during ANC AND L&D
```{r}

 # WHENY ANY TWO TIMEPOINT SBP OR DBP ARE ABOVE THE THRESHOLD (DO NOT HAVE TO BE CONSECUTIVE)
# AND 'NO' CHRONIC HYPERTENSION! 
merged_df <- merged_df %>%
  rowwise() %>%
  mutate(GEST_HYPER_MEAS_ANY2 = ifelse(((sum(is_ghtn(meanSBP_ANC20, meanDBP_ANC20),
...
table(merged_df$GEST_HYPER_MEAS_ANY2, merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$GEST_HYPER_MEAS_ANY2, merged_df$SITE, useNA = "always"), margin=2)*100,2)
```

# gHTN MEASURED @ ANY 1 TIMEPOINT during ANC AND L&D
```{r}
# AND NO CHRONIC HTN.
merged_df <- merged_df %>%
  rowwise() %>%
  mutate(GEST_HYPER_MEAS_ANY1 = ifelse(((sum(is_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                       is_ghtn(meanSBP_ANC28, meanDBP_ANC28),
                                       is_ghtn(meanSBP_ANC32, meanDBP_ANC32),
                                       is_ghtn(meanSBP_ANC36, meanDBP_ANC36),
                                       is_ghtn(meanSBP_IPC, meanDBP_IPC),
...
table(merged_df$GEST_HYPER_MEAS_ANY1, merged_df$SITE, useNA = "always")
round(prop.table(table(merged_df$GEST_HYPER_MEAS_ANY1, merged_df$SITE, useNA = "always"), margin=2)*100,2)
```


# TREATMENT FOR GESTATIONAL HTN (INCL. AT L&D)
```{r}
######################################
...
# AND NO CHRONIC HYPERTENSION AT THAT VISIT FROM FROM CLINICAL DIAGNOSIS
merged_df <- merged_df %>%                      
  mutate(GEST_HYPER_TREAT = case_when((M04_HPD_MHTERM_2!=3 & # When it is not cHTN
                                         # VISIT 20
                                         (1 %in% c(M04_HPD_OECTRT_3_2, M04_HPD_OECTRT_4_2, M04_HPD_OECTRT_5_2, # ANC
                                                   ...
                                                   M19_HDP_HTN_MHOCCUR_9_13, M19_HDP_HTN_MHOCCUR_10_13, M19_HDP_HTN_MHOCCUR_88_13,
                                                   ...
                                                   M09_HDP_HTN_PROCCUR_9_6, M09_HDP_HTN_PROCCUR_10_6, M09_HDP_HTN_PROCCUR_88_6))) ~ 1,
                                      TRUE ~ as.numeric(0)))

table(GEST_HYPER_TREAT = merged_df$GEST_HYPER_TREAT, merged_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_TREAT = merged_df$GEST_HYPER_TREAT, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)  

# temp.df <- merged_df %>% select(MOMID, PREGID, GEST_HYPER_MEAS, starts_with("M04_HTN_CMTRT_")) # I DON'T NEED THIS CHRONIC HTN MEDICATION TREATMENT DATA. 
```


# GESTATIONAL HYPERTENSION MEASURED ONCE AND TREATMENT AT ANC AND L&D
```{r}
# THIS INCLUDES NO CHRONIC HYPERTENSION IN THESE INDICATOR VARIABLES THAT ARE INCLUDED IN THIS.
merged_df <- merged_df %>%
  mutate(GEST_HYPER_MEAS1_TREAT = case_when((GEST_HYPER_MEAS_ANY1==1 & GEST_HYPER_TREAT==1) ~ 1,
                                            TRUE ~ as.numeric(0)))

table(GEST_HYPER_MEAS1_TREAT = merged_df$GEST_HYPER_MEAS1_TREAT, merged_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_MEAS1_TREAT = merged_df$GEST_HYPER_MEAS1_TREAT, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2) 

```


# SEVERE GESTATIONAL HYPERTENSION - SEVERE_GEST_HYPER_MEAS*** AT ANC & L&D
...
##################################
# THIS INCLUDES NO CHRONIC HYPERTENSION.

# if BP >160/110 just at one occasion - then severe HTN
merged_df <- merged_df %>% 
  mutate(SEVERE_GEST_HYPER_MEAS = ifelse(((sum(is_severe_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                         ...
                                         is_severe_ghtn(meanSBP_Hosp_ANC, meanDBP_Hosp_ANC), na.rm = TRUE) >=1) &
                                      CHRON_HYPER==0), 1, 0))
           
table(SEVERE_GEST_HYPER_MEAS = merged_df$SEVERE_GEST_HYPER_MEAS, merged_df$SITE, useNA = "always")
round(prop.table(table(SEVERE_GEST_HYPER_MEAS = merged_df$SEVERE_GEST_HYPER_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)           
```

# GESTAITONAL HYPERTENSION MEASURED 2+ OR (1 MEASURED & TREATMENT) OR SEVERE gHTN - WITH PROTEIN URIA (FINAL)
```{r}
# INCLUDES 'NO CHRONIC HYPERTENSION' COMPONENT HERE.
merged_df <- merged_df %>%
  mutate(GEST_HYPER = case_when((GEST_HYPER_MEAS1_TREAT==1 | GEST_HYPER_MEAS_ANY2 ==1 | 
                                  SEVERE_GEST_HYPER_MEAS ==1) ~ 1,
                                all(is.na(c(GEST_HYPER_MEAS1_TREAT, GEST_HYPER_MEAS_ANY2, SEVERE_GEST_HYPER_MEAS))) ~ as.numeric(NA), 
                                TRUE ~ as.numeric(0)))

table(GEST_HYPER = merged_df$GEST_HYPER, merged_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER = merged_df$GEST_HYPER, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2) 
```

# GESTATIONAL HYPERTENSION MEASURED 2+ OR (1 MEASURED & TREATMENT) OR SEVERE gHTN - "WITHOUT" PROTEIN URIA (FINAL)
PROTEINURIA_ANC_IPC
```{r}
merged_df <- merged_df %>%
  mutate(GEST_HYPER_WO_PROTEINURIA = case_when(GEST_HYPER==1 & (PROTEINURIA_ANC_IPC==0 | is.na(PROTEINURIA_ANC_IPC)) ~ 1,
                                                                TRUE ~ as.numeric(0)))

table(GEST_HYPER_WO_PROTEINURIA = merged_df$GEST_HYPER_WO_PROTEINURIA, merged_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_WO_PROTEINURIA = merged_df$GEST_HYPER_WO_PROTEINURIA, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
# GESTATIONAL HYPERTENSION - GEST_HYPER_DIAG***
...
```{r}
merged_df <- merged_df %>% 
  mutate(SEVERE_GEST_HYPER_DIAG = case_when(M09_HDP_HTN_MHOCCUR_3_6==1 & M09_PREECLAMPSIA_CEOCCUR_3_6==1 ~ 1,
                                           M19_HDP_HTN_MHOCCUR_3_13==1 & M19_PREECLAMPSIA_CEOCCUR_3_13==1 ~ 1,
                                           TRUE ~ as.numeric(0)))
table(SEVERE_GEST_HYPER_DIAG = merged_df$SEVERE_GEST_HYPER_DIAG, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(SEVERE_GEST_HYPER_DIAG = merged_df$SEVERE_GEST_HYPER_DIAG, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
missing_df <- merged_df %>%
  select(MOMID, PREGID, SITE, M09_DELIV_DSSTDAT_INF1_6, 
         M08_UA_PROT_LBORRES_2, M08_UA_PROT_LBORRES_3, M08_UA_PROT_LBORRES_4, 
         M08_UA_PROT_LBORRES_5, M08_UA_PROT_LBORRES_6, M08_UA_PROT_LBORRES_7,
         M08_UA_PROT_LBORRES_8, M08_UA_PROT_LBORRES_9, M08_UA_PROT_LBORRES_10,
         M08_UA_PROT_LBORRES_13, M08_UA_PROT_LBORRES_14)
```
...
# i can only measure hemolysis at week 32 with MNH08 b/c maternal bili is only checked then. 
...
#####################
merged_df <- merged_df %>%
  mutate(MAT_HEMOLYSIS = case_when((MAT_ANEMIA_MEAS4 == "severe" & M08_IBILIRUBIN_LBORRES_4 >=20.52 ) ~ 1, # M08_GAMMAGT_LBORRES_4 is not part of the hemolysis definition on slide 58.
                                    TRUE ~ as.numeric(0)))
                               
table(MAT_HEMOLYSIS = merged_df$MAT_HEMOLYSIS, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_HEMOLYSIS = merged_df$MAT_HEMOLYSIS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
...
merged_df <- merged_df %>%
  mutate(MAT_HELLP = case_when((MAT_HEMOLYSIS ==1 | M08_AST_UL_LBORRES_4 >= 90 | M08_ALT_UL_LBORRES_4 >= 120 | # AST AND ALT 2X THE UPPER NORMAL RANGE and ONLY MEASURED AT ENROLLMENT AND WEEK32.
                                           (M08_CBC_PLATE_LBORRES_4 >=0 & M08_CBC_PLATE_LBORRES_4 < 100)) ~ 1, 
         TRUE ~ as.numeric(0)))

table(MAT_HELLP = merged_df$MAT_HELLP, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_HELLP = merged_df$MAT_HELLP, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
...
# SEVERE FEATURES OF PREECLAMPISA (MEASURED)
...
##################################################################################
merged_df <- merged_df %>%
  mutate(MAT_PE_SEV_MEAS = case_when((SEVERE_GEST_HYPER_MEAS==1 |
                                           (MAT_HEMOLYSIS ==1 | # HEMOGLOBIN <7 & BILI HIGH
                                              M08_AST_UL_LBORRES_4 >= 90 | M08_ALT_UL_LBORRES_4 >= 120 | # HEPATIC ABNORMALITY: AST AND ALT 2X THE UPPER NORMAL RANGE
                                              (M08_CBC_PLATE_LBORRES_4 >=0 & M08_CBC_PLATE_LBORRES_4 < 100)) | # LOW PLATELET
                                           M08_CREAT_UMOLL_LBORRES_4 > 97.26) ~ 1, # 1.1mg/dl or 97.26umol/dl KIDNEY FUNCTION 
                                         TRUE ~ as.numeric(0)))

table(MAT_PE_SEV_MEAS = merged_df$MAT_PE_SEV_MEAS, SITE = merged_df$SITE, useNA = "always" )
round(prop.table(table(MAT_PE_SEV_MEAS = merged_df$MAT_PE_SEV_MEAS, SITE = merged_df$SITE, useNA = "always" ), margin=2)*100, 2)
                                         
```

# PREECLAMPSIA (PE) WITH cHTN (FINAL)
```{r}
##################################################################################
#TODO 
# NOTES: 
# - THIS INCLUDES cHTN + GESTATIONAL PROTEINURIA
##################################################################################

merged_df <- merged_df %>%
  mutate(MAT_PE_CHRONIC_HTN_MEAS = case_when((CHRON_HYPER ==1 & PROTEINURIA==1) ~ 1,
                                      TRUE ~ as.numeric(0)))

table(MAT_PE_CHRONIC_HTN_MEAS = merged_df$MAT_PE_CHRONIC_HTN_MEAS, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_CHRONIC_HTN_MEAS = merged_df$MAT_PE_CHRONIC_HTN_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```

# PREECLAMPSIA (PE) WITHOUT cHTN (FINAL)
```{r}
##################################################################################
#TODO 
# NOTES: 
# - THIS INCLUDES gHTN + GESTATIONAL PROTEINURIA
##################################################################################

merged_df <- merged_df %>%
  mutate(MAT_PE_GEST_HTN_MEAS = case_when((GEST_HYPER_MEAS ==1 & PROTEINURIA==1) ~ 1,
                                      TRUE ~ as.numeric(0)))

table(MAT_PE_GEST_HTN_MEAS = merged_df$MAT_PE_GEST_HTN_MEAS, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_GEST_HTN_MEAS = merged_df$MAT_PE_GEST_HTN_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# PREECLAMPSIA (PE) ANY (CHRONIC, GESTATIONAL, OR WITH SEVERE FEATURES) MEASURED (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_PE_ANY_MEAS = case_when((MAT_PE_SEV_MEAS==1 |
                                       MAT_PE_CHRONIC_HTN_MEAS==1 | 
                                       MAT_PE_GEST_HTN_MEAS==1) ~ 1,
                                     TRUE ~ as.numeric(0)))

table(MAT_PE_ANY_MEAS = merged_df$MAT_PE_ANY_MEAS, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_ANY_MEAS = merged_df$MAT_PE_ANY_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# PREECLAMPSIA MEASURED IS COMPLETE, NEED TO CREATE DIAGONSED AND SELF-REPORTED VERSIONS! 

# ORGAN DYSFUNCTION - MAT_DYS - WILL ONLY INCLUDE ORGAN DYSFUNCTION (NOT SEPSIS)
...
#TODO 
# NOTES: 
# - THIS IS LOOKING AT L&D, HOSPITALIZATION AT ANC OR PNC AND PNC SCHEDULED THROUGH VISIT 10.
##################################################################################
merged_df <- merged_df %>% 
  mutate(MAT_DYS = case_when(((1 %in% c(M09_ORG_FAIL_MHOCCUR_1_6, M09_ORG_FAIL_MHOCCUR_2_6, M09_ORG_FAIL_MHOCCUR_3_6, # ORGAN DYS DURING L&D
                                        M09_ORG_FAIL_MHOCCUR_4_6, M09_ORG_FAIL_MHOCCUR_5_6, M09_ORG_FAIL_MHOCCUR_6_6,   
                                        M09_ORG_FAIL_MHOCCUR_7_6)) & (M09_ORG_FAIL_SRCE_1_6==2 | M09_ORG_FAIL_SRCE_1_6==3)) ~ 1,
                                        
                              # HOSPITALIZATION _13
                                      (1 %in% c(M19_ORGAN_FAIL_MHTERM_1_13, M19_ORGAN_FAIL_MHTERM_2_13, M19_ORGAN_FAIL_MHTERM_3_13, 
                                        M19_ORGAN_FAIL_MHTERM_4_13, M19_ORGAN_FAIL_MHTERM_5_13, M19_ORGAN_FAIL_MHTERM_6_13,  
                                        M19_ORGAN_FAIL_MHTERM_7_13)) ~ 1, # THESE ARE 6 DIFFERENT TYPES OF ORGAN FAILURES
                              
                               # HOSPITALIZATION _14
                                      (1 %in% c(M19_ORGAN_FAIL_MHTERM_1_14, M19_ORGAN_FAIL_MHTERM_2_14, M19_ORGAN_FAIL_MHTERM_3_14, 
                                        M19_ORGAN_FAIL_MHTERM_4_14, M19_ORGAN_FAIL_MHTERM_5_14, M19_ORGAN_FAIL_MHTERM_6_14,  
                                        M19_ORGAN_FAIL_MHTERM_7_14)) ~ 1, # THESE ARE 6 DIFFERENT TYPES OF ORGAN FAILURES
                              
                              # PNC VISIT (THIS IS NOT AVAILABLE AT ANC). SHOULDN'T HAVE BEEN COLLECTED ORGAN DYS AT PNC BUT WAS LEFT IN BY MISTAKE.
                               # VISIT 7
                                         (1 %in% c(M12_ORG_FAIL_MHOCCUR_1_7, M12_ORG_FAIL_MHOCCUR_2_7,
                                                   M12_ORG_FAIL_MHOCCUR_3_7, M12_ORG_FAIL_MHOCCUR_4_7, 
                                                   M12_ORG_FAIL_MHOCCUR_88_7,
                                                   
                                                   # VISIT 8
                                                   M12_ORG_FAIL_MHOCCUR_1_8, M12_ORG_FAIL_MHOCCUR_2_8,
                                                   M12_ORG_FAIL_MHOCCUR_3_8, M12_ORG_FAIL_MHOCCUR_4_8, 
                                                   M12_ORG_FAIL_MHOCCUR_88_8,
                                                   
                                                   # VISIT 9
                                                   M12_ORG_FAIL_MHOCCUR_1_9, M12_ORG_FAIL_MHOCCUR_2_9,
                                                   M12_ORG_FAIL_MHOCCUR_3_9, M12_ORG_FAIL_MHOCCUR_4_9, 
                                                   M12_ORG_FAIL_MHOCCUR_88_9,
                                                   
                                                   # VISIT 10
                                                   M12_ORG_FAIL_MHOCCUR_1_10, M12_ORG_FAIL_MHOCCUR_2_10,
                                                   M12_ORG_FAIL_MHOCCUR_3_10, M12_ORG_FAIL_MHOCCUR_4_10, 
                                                   M12_ORG_FAIL_MHOCCUR_88_10)) ~ 1,
...
# SEVERE FEATURES OF PREECLAMPSIA - MAT_PRE_SEV_DIAG???***
...
merged_df <- merged_df %>%
  mutate(PE_DIAG = if_else(!is.na(M09_HDP_HTN_MHOCCUR_3_6) & M09_HDP_HTN_MHOCCUR_3_6==1, 1, 0))

table(PE_DIAG = merged_df$PE_DIAG, useNA = "always")
round(prop.table(table(PE_DIAG = merged_df$PE_DIAG, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
###############################################
# THIS BELOW MAY NOT BE CORRECT - USE THE ONE IN MAT_GDM_ANALYSIS.Rmd file.
merged_df <- merged_df %>% 
  ...
  mutate(GEST_DIAB_MEAS = case_when((((is.na(M08_HBA1C_PRCNT_1) | M08_HBA1C_PRCNT_1<6.5) & 
                                    ...
                                    # SHOW MISSING WHERE PRETEST GLUCOSE IS MISSING & 1-HR OGTT IS MISSING & 2-HR OGTT IS MISSING.
...
round(prop.table(table(GEST_DIAB_MEAS = merged_df$GEST_DIAB_MEAS, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)


# WOMEN WHO HAVE DON'T HAVE OVERT DIABETES
merged_df <- merged_df %>%
  mutate(OVERT_DIAB = case_when(M08_HBA1C_PRCNT_1>=6.5 ~ 1,
         TRUE ~ as.numeric(0)))
table(merged_df$OVERT_DIAB, merged_df$SITE, useNA = "always")
```


```{r}
temp.merged <- merged_df %>% filter(SITE=="Pakistan") %>% filter(ENROLLED_FF==1) %>% filter(!is.na(M08_LBSTDAT_3))

temp.merged <- merged_df %>% filter(SITE=="Pakistan") %>% distinct(MOMID)
...
tapply(merged_df$M08_HBA1C_PRCNT_1, merged_df$SITE, summary)
# %>% group_by(SITE) %>% summary(M08_HBA1C_PRCNT_1)

# HAVE YOU EVER BEEN DIAGNOSED WITH DIABETES (PRE-EXISITING OR CHRONIC) NEED FROM VISITS 3,4,5.
# SIDE 75 SAYS THIS SHOULD BE VISITS 3-5
merged_df <- merged_df %>% 
  mutate(GEST_DIAB_DIAG= case_when((M04_DIABETES_EVER_MHOCCUR_3==0 | M04_DIABETES_EVER_MHOCCUR_4==0 | M04_DIABETES_EVER_MHOCCUR_5==0) &
                                      ((M09_GEST_DIAB_MHOCCUR_6==1 | # WAS MOTHER DIAGNOSED WITH DIABETES
...
```{r}
mnh08_df <- mnh_list[["mnh08"]]
merged_df2 <- merged_df %>% select(MOMID, PREGID, SITE, ENROLLED_FF)

dup <- mnh08_df[duplicated(mnh08_df),]
mnh08_df2 <- mnh08_df %>% filter(!is.na(MOMID)) %>% #try using 'drop_na (variables)
  distinct (MOMID, PREGID, SITE, M08_TYPE_VISIT, .keep_all = TRUE) # keep_all keeps the first row rather than dropping both duplicates.

mnh08_df3 <- left_join(mnh08_df2, merged_df2, by = c("MOMID", "PREGID", "SITE"))
mnh08


#mnh08_df2 <- mnh08_df
test.df <- mnh08_df3 %>% filter(M08_TYPE_VISIT==3 & M08_HBA1C_PRCNT<6.5 & ENROLLED_FF==1) %>%
  mutate(GEST_DIAB_MEAS = case_when((((is.na(M08_HBA1C_PRCNT_1) | M08_HBA1C_PRCNT_1<6.5) & 
                                        ((M08_BGLUC_PRETEST_MMOLL_LBORRES_3>=5.1 | # confirm pre test (fasting) OGTT is high at ANC28 OR
                                           # when fasting OGTT is high, then 1-hr or 2-hr should be also high at ANC28 OR
                                           M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_3>=10.0) |  # 1-hr at ANC28
                                        M08_BGLUC_ORAL_2HR_MMOLL_LBORRES_3 >=8.5))) ~ 1,
...
# THIS IS WHEN I WAS LOOKING AT -5 AND -7 VALUES FOR MISSNG BEFORE CONVERTING TO NA.
   # mutate(GEST_DIAB_MEAS = case_when((((!is.na(M08_HBA1C_PRCNT) & M08_HBA1C_PRCNT<6.5 & M08_HBA1C_PRCNT!=-5 ) & 
   #                                      (M08_BGLUC_PRETEST_MMOLL_LBORRES>=5.1 | # confirm pre test (fasting) OGTT is high at ANC28
   #                                       # when fasting OGTT is high, then 1-hr or 2-hr should be also high at ANC28
   #                                       M08_BGLUC_ORAL_1HR_MMOLL_LBORRES>=10.0) |  # 1-hr at ANC28
   #                                    M08_BGLUC_ORAL_2HR_MMOLL_LBORRES >=8.5)) ~ 1,
   #                                  
   #                               #  (is.na(M06_BGLUC_POC_MMOLL_LBORRES) &
   #                                     is.na(M08_BGLUC_PRETEST_MMOLL_LBORRES) & is.na(M08_BGLUC_ORAL_1HR_MMOLL_LBORRES) &
   #                                      is.na(M08_BGLUC_ORAL_2HR_MMOLL_LBORRES) ~ as.numeric(NA),
   #       TRUE ~ as.numeric(0))) # 2-hr

table(GEST_DIAB_MEAS = test.df$GEST_DIAB_MEAS, SITE = test.df$SITE, useNA = "always")
round(prop.table(table(GEST_DIAB_MEAS = test.df$GEST_DIAB_MEAS, SITE = test.df$SITE, useNA = "always"), margin=2)*100,2)

# overt diabetes:
test.df <- mnh08_df %>% filter(M08_TYPE_VISIT==1) %>%
  mutate(overt_diabetes = case_when(M08_HBA1C_PRCNT>6.5 ~ 1,
         TRUE ~ as.numeric(0)))
table(test.df$overt_diabetes, test.df$SITE, useNA = "always")

# OGTT: 
test.df <- mnh08_df %>% filter(M08_TYPE_VISIT==3) %>%
  mutate(OGTT_PRE = case_when(M08_BGLUC_PRETEST_MMOLL_LBORRES>5.1 ~ 1,
         TRUE ~ as.numeric(0)))

# OGTT 1-hr: 
test.df <- mnh08_df %>% filter(M08_TYPE_VISIT==3) %>% filter(SITE=="Kenya") %>%
  mutate(OGTT_1hr = case_when(M08_HBA1C_PRCNT<6.5 & M08_BGLUC_ORAL_1HR_MMOLL_LBORRES>=10.0 ~ 1,
         TRUE ~ as.numeric(0))) %>%
...
# OGTT 2-hr: 
test.df <- mnh08_df %>% filter(M08_TYPE_VISIT==3) 
  mutate(OGTT_2hr = case_when(M08_BGLUC_ORAL_2HR_MMOLL_LBORRES >=8.5 ~ 1,
         TRUE ~ as.numeric(0)))
...
write.csv(diabetes_df, 'data_out/mat_diabetes.csv')
```
...
temp.df <- merged_df %>% 
  select(MOMID, PREGID, 
         M04_APH_CEOCCUR_3, M04_APH_CEOCCUR_4, 
         M04_APH_CEOCCUR_5, M09_APH_CEOCCUR_6, 
         M19_VAG_BLEED_CEOCCUR_13, M19_LD_COMPL_MHTERM_4_13) 
...
# SECONDARY: MATERNAL INFECTION AND SEPSIS (PREGNANCY)
...
# MATERNAL INFECTION DIAGNOSED - WITHOUT HOSP (ANC) 
```{r}
#################################################################################
#TODO 
# NOTES: 
...
# MATERNAL INFECTION DIAGNOSED AT ANC - HOSPITALIZATION _13
```{r}
...
table(MAT_INFECT_DIAG_ANC = merged_df$MAT_INFECT_DIAG_ANC, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_DIAG_ANC = merged_df$MAT_INFECT_DIAG_ANC, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)

```

# MATERNAL INFECTION AT DIAGNOSIS THORUGH TREATMENT MEDICATION USE AT ANC
```{r}
#############################################
...
# DOESN'T INCLUDE UNSCHEDULED ANC VISITS YET! 
```{r}
###########################################################################
# MATERNAL INFECTION MEASURED THROUGH POC MNH06 and LAB MNH06 - ANC ONLY #
#TODO DOESN'T INCLUDE UNSCHEDULED ANC VISITS YET! 
##########################################################################
...
# ------ NOTE THAT ORGAN DYSFUCTION OR SEPSIS IS NOT DONE IN MNH04 (CLINCIAL STATUS), MNH06 (POC DIAGNOSTICS), OR MNH08 (MATERNAL LABS) FOR ANC.  IT IS ONLY DURING HOSPITALIZATION
```{r}

######################################
#TODO
# - SEPSIS/ORGAN DYSFUNCTION INFO IS ONLY PRESENT AT HOSPITALIZATION FOR ANC IN MNH19
# - THERE IS NO SEPSIS OR ORGAN DYSFUCTION INFO IN MNH04, MNH06 OR MNH08
######################################
...
# MATERNAL INFECTION DIAGNOSIS - PNC VISIT
# -------DOESN'T INLCUDE HOSPITALIZATION _14 DURING PNC YET
...
merged_df <- merged_df %>%                  # VISIT 7
  mutate(MAT_INFECT_DIAG_PNC_WO_HOSP = case_when((1 %in% c(M12_POST_DEL_INF_MHOCCUR_1_7, M12_POST_DEL_INF_MHOCCUR_2_7, M12_POST_DEL_INF_MHOCCUR_3_7,
                                                            M12_POST_DEL_INF_MHOCCUR_4_7, M12_POST_DEL_INF_MHOCCUR_5_7, M12_POST_DEL_INF_MHOCCUR_6_7, 
                                                            M12_POST_DEL_INF_MHOCCUR_7_7, M12_POST_DEL_INF_MHOCCUR_8_7, M12_POST_DEL_INF_MHOCCUR_9_7,
                                                            M12_POST_DEL_INF_MHOCCUR_10_7)) ~1, # SEPSIS NOT INCLUDED HERE.
                                                  ...
                                                  # VISIT 8: 
                                                  (1 %in% c(M12_POST_DEL_INF_MHOCCUR_1_8, M12_POST_DEL_INF_MHOCCUR_2_8, M12_POST_DEL_INF_MHOCCUR_3_8,
                                                            M12_POST_DEL_INF_MHOCCUR_4_8, M12_POST_DEL_INF_MHOCCUR_5_8, M12_POST_DEL_INF_MHOCCUR_6_8, 
                                                            M12_POST_DEL_INF_MHOCCUR_7_8, M12_POST_DEL_INF_MHOCCUR_8_8, M12_POST_DEL_INF_MHOCCUR_9_8,
                                                            M12_POST_DEL_INF_MHOCCUR_10_8)) ~ 1,
                                                  ...
                                                  # VISIT 9:
                                                  (1 %in% c(M12_POST_DEL_INF_MHOCCUR_1_9, M12_POST_DEL_INF_MHOCCUR_2_9, M12_POST_DEL_INF_MHOCCUR_3_9,
                                                            M12_POST_DEL_INF_MHOCCUR_4_9, M12_POST_DEL_INF_MHOCCUR_5_9, M12_POST_DEL_INF_MHOCCUR_6_9, 
                                                            M12_POST_DEL_INF_MHOCCUR_7_9, M12_POST_DEL_INF_MHOCCUR_8_9, M12_POST_DEL_INF_MHOCCUR_9_9,
                                                            M12_POST_DEL_INF_MHOCCUR_10_9)) ~ 1,
                                                  ...
                                                  # VISIT 10: 
                                                  (1 %in% c(M12_POST_DEL_INF_MHOCCUR_1_10, M12_POST_DEL_INF_MHOCCUR_2_10, M12_POST_DEL_INF_MHOCCUR_3_10,
                                                            M12_POST_DEL_INF_MHOCCUR_4_10, M12_POST_DEL_INF_MHOCCUR_5_10, M12_POST_DEL_INF_MHOCCUR_6_10, 
                                                            M12_POST_DEL_INF_MHOCCUR_7_10, M12_POST_DEL_INF_MHOCCUR_8_10, M12_POST_DEL_INF_MHOCCUR_9_10,
                                                            M12_POST_DEL_INF_MHOCCUR_10_10)) ~ 1,
...
table(MAT_INFECT_DIAG_PNC_WO_HOSP = merged_df$MAT_INFECT_DIAG_PNC_WO_HOSP, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_DIAG_PNC_WO_HOSP = merged_df$MAT_INFECT_DIAG_PNC_WO_HOSP, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```

# MATERNAL INFECTION DURING PNC DIAGNOSED DURING HOSPITALIZATION _14
```{r}
################# PNC HOSPITALIZATION _14.
merged_df <- merged_df %>%
  mutate(MAT_INFECT_DIAG_PNC_HOSP = case_when(M19_PRIMARY_MHTERM_14==2 | # 2= INFECTION; 5= ORGAN FAILURE
                                                (1 %in% c(M19_INFECTION_MHTERM_1_14, M19_INFECTION_MHTERM_2_14, M19_INFECTION_MHTERM_3_14, 
                                                          M19_INFECTION_MHTERM_4_14, M19_INFECTION_MHTERM_5_14, M19_INFECTION_MHTERM_6_14, 
                                                          M19_INFECTION_MHTERM_7_14, M19_INFECTION_MHTERM_8_14, M19_INFECTION_MHTERM_9_14, 
                                                          M19_INFECTION_MHTERM_10_14, M19_INFECTION_MHTERM_11_14, # NOT INCLUDING 12 B/C IT IS MATERNAL SEPSIS 
                                                          M19_INFECTION_MHTERM_13_14, M19_INFECTION_MHTERM_14_14, M19_INFECTION_MHTERM_15_14, 
                                                          M19_INFECTION_MHTERM_16_14,
                                                          M19_INFECTION_MHTERM_17_14, M19_INFECTION_MHTERM_18_14, M19_INFECTION_MHTERM_19_14, 
                                                          M19_INFECTION_MHTERM_88_14)) ~ 1, 
                                              TRUE ~ as.numeric(0))) 

table(MAT_INFECT_DIAG_PNC_HOSP = merged_df$MAT_INFECT_DIAG_PNC_HOSP, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_DIAG_PNC_HOSP = merged_df$MAT_INFECT_DIAG_PNC_HOSP, 
                       SITE = merged_df$SITE, useNA = "always"),margin=2)*100,2)
```

# MATERNAL INFECTION DIAGNOSED DURING PNC INCL. HOSPITALIZATION (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECT_DIAG_PNC = case_when(1 %in% c(MAT_INFECT_DIAG_PNC_WO_HOSP, MAT_INFECT_DIAG_PNC_HOSP) ~ 1,
                                         TRUE ~ as.numeric(0)))

table(MAT_INFECT_DIAG_PNC = merged_df$MAT_INFECT_DIAG_PNC, SITE = merged_df$SITE, useNA = "always")

round(prop.table(table(MAT_INFECT_DIAG_PNC = merged_df$MAT_INFECT_DIAG_PNC, 
                       SITE = merged_df$SITE, useNA = "always"),margin=2)*100,2)

```

# MATERNAL INFECTION TREATEMENT - PNC (FINAL)
# -----DOESN'T INLCUDE HOSPITALIZATION _14 DURING PNC YET
```{r}
##########################################################################
...
# SPECIFIC MALARIA OR TB ETC TREATMENT IS NOT MENTIONED IN M12, IT ONLY HAVE HIV SPECIFIC TREATMENTS LISTED. 
merged_df <- merged_df %>% #VISIT 7
  mutate(MAT_INFECT_TREAT_PNC_WO_HOSP = case_when((M12_HIV_CMOCCUR_7==1 | !is.na(M12_HIV_CMOCCUR_DAT_7)) | 
...
table(MAT_INFECT_TREAT_PNC_WO_HOSP = merged_df$MAT_INFECT_TREAT_PNC_WO_HOSP, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_TREAT_PNC_WO_HOSP = merged_df$MAT_INFECT_TREAT_PNC_WO_HOSP, SITE = merged_df$SITE, useNA = "always"),margin=2)*100,2)
```

# MATERNAL INFECTION TREATMENT DURING PNC HOSPITALIZATION 
```{r}
##########################################################################
# MATERNAL INFECTION TREATMENT DURING HOSPITALIZATION # 
#TODo
#########################################################################
merged_df <- merged_df %>%
  mutate(MAT_INFECT_TREAT_PNC_HOSP = case_when(M19_ABX_CMOCCUR_14==1 | 
                                                     ((1 %in% c(M19_ABX_CMTRT_1_14, M19_ABX_CMTRT_2_14, M19_ABX_CMTRT_3_14, 
                                                                M19_ABX_CMTRT_4_14, M19_ABX_CMTRT_5_14, M19_ABX_CMTRT_6_14,
                                                                M19_ABX_CMTRT_7_14, M19_ABX_CMTRT_8_14, M19_ABX_CMTRT_9_14,
                                                                M19_ABX_CMTRT_88_14)) & M19_ABX_CMINDC_14!=8) ~ 1, # 'DON'T KNOW' IS NOT INCLUDED.
                                                   TRUE ~ as.numeric(0)))

table(MAT_INFECT_TREAT_PNC_HOSP = merged_df$MAT_INFECT_TREAT_PNC_HOSP, SITE = merged_df$SITE, useNA = "always")

round(prop.table(table(MAT_INFECT_TREAT_PNC_HOSP = merged_df$MAT_INFECT_TREAT_PNC_HOSP, 
                       SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```
...
merged_df <- merged_df %>%
  mutate(MAT_INFECT_TREAT_PNC = case_when(1 %in% c(MAT_INFECT_TREAT_PNC_WO_HOSP, MAT_INFECT_TREAT_PNC_HOSP) ~ 1,
         TRUE ~ as.numeric(0)))

table(MAT_INFECT_TREAT_PNC = merged_df$MAT_INFECT_TREAT_PNC, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_TREAT_PNC = merged_df$MAT_INFECT_TREAT_PNC, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```


# MATERNAL INFECTION DURING PNC - MEASURED (FINAL)
# ------ DOESN'T INCLUDE UNSCHEDULED ANC VISITS YET! 
```{r}
##########################################################################
# MATERNAL INFECTION DURING PNC - MEASURED # 
#TODO
# MNH08 DURING PNC6 ONLY - VISIT 10
# MNH 06 DURING PNC0, PNC1, PNC4, PNC6 (VISITS 7-10)
# DOESN'T INCLUDE HOSPITALIZATION DURING PNC YET.
#########################################################################

# LAB DIAGNOSED (MNH08) - LABS ARE NOT DONE DURING HOSPITALIZATION _14 VISITS.
# BUT MNH06 IS DONE DURING HOSPITALZIATION ( ASK ERS?)

merged_df <- merged_df %>%  # POC FIRST (MNH06)
  mutate(MAT_INFECT_MEAS_PNC = case_when((M06_HIV_POC_LBORRES_7==1 | M06_HIV_POC_LBORRES_8==1 | M06_HIV_POC_LBORRES_9==1 | 
                                            M06_HIV_POC_LBORRES_10==1 | # M06_HIV_POC_LBORRES_14==1 | # UNSCHEDULED ANC VISITS _14
                                            
                                            M06_MALARIA_POC_LBORRES_7==1 | M06_MALARIA_POC_LBORRES_8==1 | M06_MALARIA_POC_LBORRES_9==1 |
                                            M06_MALARIA_POC_LBORRES_10==1 | # M06_MALARIA_POC_LBORRES_14==1 | 
                                            
                                            M06_SYPH_POC_LBORRES_7==1 | M06_SYPH_POC_LBORRES_8==1 | M06_SYPH_POC_LBORRES_9==1 |
                                            M06_SYPH_POC_LBORRES_10==1 | # M06_SYPH_POC_LBORRES_14==1 |
                                            
                                            M06_HBV_POC_LBORRES_7==1 | M06_HBV_POC_LBORRES_8==1 | M06_HBV_POC_LBORRES_9==1 | 
                                            M06_HBV_POC_LBORRES_10==1 | # M06_HBV_POC_LBORRES_14==1 |
                                            
                                            M06_HCV_POC_LBORRES_7==1 | M06_HCV_POC_LBORRES_8==1 | M06_HCV_POC_LBORRES_9==1 | 
                                            M06_HCV_POC_LBORRES_10==1 | # M06_HCV_POC_LBORRES_14==1 | 
                                            
                                            M06_COVID_POC_LBORRES_7==1 | M06_COVID_POC_LBORRES_8==1 | M06_COVID_POC_LBORRES_9==1 | 
                                            M06_COVID_POC_LBORRES_10==1) ~ 1, #| M06_COVID_POC_LBORRES_14==1 
                                         
                                         # MNH08  
                                         (1 %in% c(M08_MALBL_LBORRES_10, M08_PLACMAL_LBPERF_1_10, # MNH08 DONE ANLY DURING PNC6 (VISIT 10)
...
table(MAT_INFECT_MEAS_PNC = merged_df$MAT_INFECT_MEAS_PNC, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_MEAS_PNC = merged_df$MAT_INFECT_MEAS_PNC, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```


# MATERNAL SEPSIS DIAGNOSED (INCLUDING ORGAN DYS) AT PNC INCLUDING HOSPITALIZATION _14 (FINAL)
# ------ NOTE THAT ORGAN DYSFUCTION OR SEPSIS IS NOT DONE IN MNH06 (POC DIAGNOSITICS), MNH08 (MATERNAL LABS) FOR PNC.  
# --------------IT IS ONLY IN OR MNH12 (CLINICAL STATUS) AND DURING HOSPITALIZATION
```{r}

######################################
#TODO
# - SEPSIS/ORGAN DYSFUNCTION INFO IS ONLY PRESENT AT HOSPITALIZATION FOR PNC IN MNH12 AND MNH19 (HOSPITALIZATION _14)
# - THERE IS NO SEPSIS INFO IN MNH04, MNH06 OR MNH08
######################################


merged_df <- merged_df %>%
  mutate(MAT_SEPSIS_DIAG_PNC = case_when((1 %in% c(M12_POST_DEL_INF_MHOCCUR_11_7, M12_POST_DEL_INF_MHOCCUR_11_8, # MATERNAL SEPSIS DURING MNH12
                                                  M12_POST_DEL_INF_MHOCCUR_11_9, M12_POST_DEL_INF_MHOCCUR_11_10)) ~ 1,
                                         ...
                                         (M19_PRIMARY_MHTERM_14==5 | # DURING HOSPITALIZATION
                                             (1 %in% c(M19_ORGAN_FAIL_MHTERM_1_14, M19_ORGAN_FAIL_MHTERM_2_14, 
                                                       M19_ORGAN_FAIL_MHTERM_3_14, M19_ORGAN_FAIL_MHTERM_4_14, 
                                                       M19_ORGAN_FAIL_MHTERM_5_14, M19_ORGAN_FAIL_MHTERM_6_14, 
                                                       M19_ORGAN_FAIL_MHTERM_7_14, 
                                                       M19_ORGAN_FAIL_MHTERM_88_14, M19_ORGAN_FAIL_MHTERM_99_14))) ~ 1, # OTHER AND DON'T KNOW.
                                         M19_INFECTION_MHTERM_12_14==1 ~ 1, # MATERNAL SEPSIS OPTION ON MNH19
                                         TRUE ~ as.numeric(0)))


table(MAT_SEPSIS_DIAG_PNC = merged_df$MAT_SEPSIS_DIAG_PNC, SITE = merged_df$SITE, useNA = "always") 
round(prop.table(table(MAT_SEPSIS_DIAG_PNC = merged_df$MAT_SEPSIS_DIAG_PNC, SITE = merged_df$SITE, useNA = "always"),margin=2)*100,2)
```

# MATERNAL INFECTIONSEPSIS_ANY_PNC - ALL PNC (INFECT DIAGNOSED, INFECT TREATED, INFECT MEASURED + SEPSIS DIAGNOSED) (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECTIONSEPSIS_ANY_PNC = case_when(1 %in% c(MAT_INFECT_DIAG_PNC, MAT_INFECT_TREAT_PNC, MAT_INFECT_MEAS_PNC, MAT_SEPSIS_DIAG_PNC) ~ 1,
         TRUE ~ as.numeric(0)))

table(MAT_INFECTIONSEPSIS_ANY_PNC = merged_df$MAT_INFECTIONSEPSIS_ANY_PNC, SITE = merged_df$SITE, useNA = "always") 
round(prop.table(table(MAT_INFECTIONSEPSIS_ANY_PNC = merged_df$MAT_INFECTIONSEPSIS_ANY_PNC, SITE = merged_df$SITE, useNA = "always"),margin=2)*100,2)

```

# MATERNAL INFECT_DIAGNOSED AT ANYTIME (ANC, ANC-HOSP, L&D, PNC, PNC-HOSP) _ANYTIME (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECT_DIAG_ANYTIME = case_when(1 %in% c(MAT_INFECT_DIAG_ANC, MAT_INFECT_DIAG_IPC, MAT_INFECT_DIAG_PNC) ~ 1,
                                             TRUE ~ as.numeric(0)))

table(MAT_INFECT_DIAG_ANYTIME = merged_df$MAT_INFECT_DIAG_ANYTIME, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_DIAG_ANYTIME = merged_df$MAT_INFECT_DIAG_ANYTIME, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)
```

# MATERNAL INFECTION TREAT AT ANC (& HOSP _13), L&D, PNC (& HOSP _14) _ANYTIME (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECT_TREAT_ANYTIME = case_when((1 %in% c(MAT_INFECT_TREAT_ANC, MAT_INFECT_TREAT_IPC, MAT_INFECT_TREAT_PNC)) ~ 1,
                                             TRUE ~ as.numeric(0)))

table(MAT_INFECT_TREAT_ANYTIME = merged_df$MAT_INFECT_TREAT_ANYTIME, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_TREAT_ANYTIME = merged_df$MAT_INFECT_TREAT_ANYTIME, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# MATERNAL INFECTION MEAS AT ANC (& HOSP _13), AND PNC (& HOSP _14) _ANYTIME - (L&D DOESN'T HAVE THIS MEASUREMENT) (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECT_MEAS_ANYTIME = case_when((1 %in% c(MAT_INFECT_MEAS_ANC, MAT_INFECT_MEAS_PNC)) ~ 1, # NO MEASUREMENT AT IPC. 
                                             TRUE ~ as.numeric(0)))

table(MAT_INFECT_MEAS_ANYTIME = merged_df$MAT_INFECT_MEAS_ANYTIME, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECT_MEAS_ANYTIME = merged_df$MAT_INFECT_MEAS_ANYTIME, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# MATERNAL SEPSIS DIAGNOSED AT ANC (& HOSP _13), L&D, PNC (& HOSP _14) _ANYTIME (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_SEPSIS_DIAG_ANYTIME = case_when((1 %in% c(MAT_SEPSIS_DIAG_ANC, MAT_INFECT_TREAT_IPC, MAT_SEPSIS_DIAG_PNC)) ~ 1, # NO MEASUREMENT AT IPC. 
                                             TRUE ~ as.numeric(0)))

table(MAT_SEPSIS_DIAG_ANYTIME = merged_df$MAT_SEPSIS_DIAG_ANYTIME, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_SEPSIS_DIAG_ANYTIME = merged_df$MAT_SEPSIS_DIAG_ANYTIME, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```

# MATERNAL INFECTION & SEPSIS BY ANY METHOD (DIAGNOSED, TREAT, MEASURED) AND AT ANY TIME POINT (ANC (& HOSP _13), L&D, PNC (& HOSP _14)) _ANYTIME (FINAL)
```{r}
merged_df <- merged_df %>%
  mutate(MAT_INFECTIONSEPSIS_ANY_ANYTIME = case_when((1 %in% c(MAT_INFECTIONSEPSIS_ANY_ANC, MAT_INFECT_TREAT_IPC, MAT_INFECTIONSEPSIS_ANY_PNC)) ~ 1,
                                             TRUE ~ as.numeric(0)))

table(MAT_INFECTIONSEPSIS_ANY_ANYTIME = merged_df$MAT_INFECTIONSEPSIS_ANY_ANYTIME, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECTIONSEPSIS_ANY_ANYTIME = merged_df$MAT_INFECTIONSEPSIS_ANY_ANYTIME, SITE = merged_df$SITE, useNA = "always"), margin=2)*100, 2)

```


#################
# WHO MATERNAL NEAR-MISS (MNM): 
# Woman who nearly died but survived a life-threatening condition (i.e. organ dysfunction) that occurred during pregnancy, childbirth or within 42 days of termination of pregnancy, irrespective of gestational age and including abortions. 

# CONSTRUCTED FROM: MAT_DEATH AND MAT_DYS.  I AM GOING TO USE SEPSIS VARIABLES THAT I CREATED UNDER INFECTION AND SEPSIS INSTEAD. 
```{r}
merged_df <- merged_df %>%
  mutate(MAT_MNM = case_when((MAT_DEATH==0 & MAT_SEPSIS_DIAG_ANYTIME==1) ~ 1, 
                             TRUE ~ as.numeric(0)))

table(MAT_MNM = merged_df$MAT_MNM, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_MNM = merged_df$MAT_MNM, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```



# 1. MATERNAL INFECTION & SEPSIS - SEVERE: ANY TYPE (DIAGNOSED, TREAT, MEASURED) AT ANC.
# Infection-related severe maternal outcomes: infection + near-miss criteria or maternal death.
```{r}
######################################
#TODO
# - DOES THIS MATERNAL DEATH ALSO INCLUDE LATER MATERNAL DEATH
# - THIS VAR NEEDS FEEDBACK FROM ERS
######################################
merged_df <- merged_df %>%
  mutate(MAT_INFECTIONSEPSIS_ANY_SEVERE = case_when((MAT_INFECTIONSEPSIS_ANY_ANC==1 & MAT_MNM==1) ~ 1, # ADD NEAR-MISS OR MATERNAL DEATH.
         TRUE ~ as.numeric(0)))

table(MAT_INFECTIONSEPSIS_ANY_SEVERE = merged_df$MAT_INFECTIONSEPSIS_ANY_SEVERE, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_INFECTIONSEPSIS_ANY_SEVERE = merged_df$MAT_INFECTIONSEPSIS_ANY_SEVERE, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```

# SEVERE COMPLICATIONS: MAT_SEV_COMP
Clinical conditions that can threaten a woman’s life during pregnancy and labour and after termination of pregnancy. Per the WHO (2011) near-miss guide: severe postpartum hemorrhage, severe preeclampsia/eclampsia, sepsis/severe systemic infection, and ruptured uterus.

All constructed vars used here: 
MAT_INFECT	Maternal infection and sepsis (any)
MAT_DYS	Life-threatening condition (i.e. organ dysfunction)
MAT_PE_SEV_MEAS	Severe features of preeclampsia (measured)
# MAT_PE_SEV_DIAG	Severe features of preeclampsia (diagnosed)
MAT_PPH_SEV	Hemorrhage (severe postpartum)
MAT_UTER_RUP	Uterine rupture

```{r}
######################################
#TODO
# - This includes maternal infection and sepsis as part of its definition so when we look at 'Maternal infection and sepsis - less severe', not sure that we can capture less severe

# - MAT_INFECTIONSEPSIS_ANY_ANYTIME variable has MAT_DYS already included. 
# - Do we want to leave out MAT_PE_SEV_DIAG out for now - all the other vars a MEASURED only the way I am creating this variable.
# - THIS VAR NEEDS FEEDBACK FROM ERS
# - NOT COMPLETE YET
######################################

merged_df <- merged_df %>%
  mutate(MAT_SEV_COMP = case_when((MAT_INFECTIONSEPSIS_ANY_ANYTIME == 1 | MAT_DYS==1 | 
                                     MAT_PE_SEV_MEAS ==1 | MAT_PPH_SEV == 1 | MAT_UTER_RUP == 1) ~ 1,
                                  TRUE ~ as.numeric(0)))

table(MAT_SEV_COMP = merged_df$MAT_SEV_COMP, SITE = merged_df$SITE, useNA = "always")
round(prop.table(table(MAT_SEV_COMP = merged_df$MAT_SEV_COMP, SITE = merged_df$SITE, useNA = "always"), margin=2)*100,2)
```

# 2. MATERNAL INFECTION & SEPSIS - COMPICATIONS: ANY TYPE (DIAGNOSED, TREAT, MEASURED) AT ANC 

# Infections with complications: infection + invasive procedure, ICU admission to ICU, or facility transfer.
# This will be complex - b/c SEVERE COMPLICATIONS variable includes MAT_INFECT, MAT_DYS, PE, PPH, UTERINE RUPTURE.
```{r}
######################################
#TODO
# - THIS VAR IS CURRENTLY INCOMPLETE.
# - NEED TO CONSTRUCT VARIABLE MAT_SEV_COMP
######################################
merged_df <- merged_df %>%
  mutate(MAT_INFECTIONSEPSIS_ANY_COMPLICATIONS = case_when((MAT_INFECTIONSEPSIS_ANY_ANYTIME==1 & 
                                                              (MAT_SEV_COMP==1 | M09_PPH_FAORRES_5_6==1 |
                                                                 (M09_CES_PROCCUR_INF1_ %in% c(1,99)) |
                                                                 M10_MAT_DSTERM_6==1 | M19_TX_PROCCUR_2_13==1 | 
                                                                 M19_TX_PROCCUR_3_13==1 | M19_TX_PROCCUR_5_13==1 | 
                                                                 M19_TX_PROCCUR_2_14==1 | 
                                                                 M19_TX_PROCCUR_3_14==1 | M19_TX_PROCCUR_5_14==1)) ~ 1,  
                                                           TRUE ~ as.numeric(0))) #TODO not complete yet.

```

# 3. MATERNAL INFECTION & SEPSIS - LESS SEVERE: ANY TYPE (DIAGNOSED, TREAT, MEASURED) AT ANC 
# Less severe infections: all other infections, subcategorized as chronic or acute infections. 
```{r}
######################################
#TODO
# - THIS VAR IS CURRENTLY INCOMPLETE.
######################################
merged_df <- merged_df %>%
  mutate(MAT_INFECTIONSEPSIS_ANY_LESSSEVERE = case_when(MAT_INFECTIONSEPSIS_ANY_ANC==1 & 
                                                          !(MAT_INFECTIONSEPSIS_ANY_SEVERE==1 | MAT_INFECTIONSEPSIS_ANY_COMPLICATIONS==1) ~ 1, # 
         TRUE ~ as.numeric(0)))
...
```{r}
export_df <- merged_df %>%
  select(MOMID, SITE, MAT_PE_ANY_MEAS, MAT_PE_SEV_MEAS, MAT_PE_CHRONIC_HTN_MEAS, 
         MAT_PE_GEST_HTN_MEAS, PROTEINURIA, CHRON_HYPER, 
         SEVERE_GEST_HYPER_DIAG, MAT_HEMOLYSIS, M08_AST_UL_LBORRES_4, M08_ALT_UL_LBORRES_4,
         M08_CBC_PLATE_LBORRES_4, M08_CBC_PLATE_LBORRES_4, M08_CREAT_UMOLL_LBORRES_4,

MAT_HELLP, MAT_HEMOLYSIS, M08_AST_UL_LBORRES_4, M08_ALT_UL_LBORRES_4,  M08_CBC_PLATE_LBORRES_4,
MAT_HEMOLYSIS, MAT_ANEMIA_MEAS4, M08_IBILIRUBIN_LBORRES_4, M08_GAMMAGT_LBORRES_2,
PROTEINURIA,M08_UA_PROT_LBORRES_2, M08_UA_PROT_LBORRES_3, M08_UA_PROT_LBORRES_4,
M08_UA_PROT_LBORRES_5, M08_UA_PROT_LBORRES_6, M08_UA_PROT_LBORRES_7,
M08_UA_PROT_LBORRES_8, M08_UA_PROT_LBORRES_9, M08_UA_PROT_LBORRES_10,
M08_UA_PROT_LBORRES_13, M08_UA_PROT_LBORRES_14,

CHRON_HYPER, meanSBP_ANC_LT20, meanDBP_ANC_LT20, 
meanSBP_ANC_LT20, M06_BP_SYS_VSORRES_1_1, M06_BP_SYS_VSORRES_2_1, M06_BP_SYS_VSORRES_3_1,
meanDBP_ANC_LT20, M06_BP_DIA_VSORRES_1_1, M06_BP_DIA_VSORRES_2_1, M06_BP_DIA_VSORRES_3_1,

GEST_HYPER_MEAS,meanSBP_ANC20, M06_BP_SYS_VSORRES_1_2, M06_BP_SYS_VSORRES_2_2, M06_BP_SYS_VSORRES_3_2,
meanDBP_ANC20, M06_BP_DIA_VSORRES_1_2, M06_BP_DIA_VSORRES_2_2, M06_BP_DIA_VSORRES_3_2,
meanSBP_ANC28, M06_BP_SYS_VSORRES_1_3, M06_BP_SYS_VSORRES_2_3, M06_BP_SYS_VSORRES_3_3,
meanDBP_ANC28, M06_BP_DIA_VSORRES_1_3, M06_BP_DIA_VSORRES_2_3, M06_BP_DIA_VSORRES_3_3,
meanSBP_ANC32, M06_BP_SYS_VSORRES_1_4, M06_BP_SYS_VSORRES_2_4, M06_BP_SYS_VSORRES_3_4,
meanDBP_ANC32, M06_BP_DIA_VSORRES_1_4, M06_BP_DIA_VSORRES_2_4, M06_BP_DIA_VSORRES_3_4,
meanSBP_ANC36, M06_BP_SYS_VSORRES_1_5, M06_BP_SYS_VSORRES_2_5, M06_BP_SYS_VSORRES_3_5,
meanDBP_ANC36, M06_BP_DIA_VSORRES_1_5, M06_BP_DIA_VSORRES_2_5, M06_BP_DIA_VSORRES_3_5,
meanSBP_IPC, M06_BP_SYS_VSORRES_1_6, M06_BP_SYS_VSORRES_2_6, M06_BP_SYS_VSORRES_3_6,
meanDBP_IPC, M06_BP_DIA_VSORRES_1_6, M06_BP_DIA_VSORRES_2_6, M06_BP_DIA_VSORRES_3_6,
meanSBP_Hosp_ANC, M19_BP_SYS_VSORRES_13,
meanDBP_Hosp_ANC = M19_BP_DIA_VSORRES_13,
meanSBP_PNC0, M06_BP_SYS_VSORRES_1_7, M06_BP_SYS_VSORRES_2_7, M06_BP_SYS_VSORRES_3_7,
meanDBP_PNC0, M06_BP_DIA_VSORRES_1_7, M06_BP_DIA_VSORRES_2_7, M06_BP_DIA_VSORRES_3_7,
meanSBP_PNC1, M06_BP_SYS_VSORRES_1_8, M06_BP_SYS_VSORRES_2_8, M06_BP_SYS_VSORRES_3_8,
meanDBP_PNC1, M06_BP_DIA_VSORRES_1_8, M06_BP_DIA_VSORRES_2_8, M06_BP_DIA_VSORRES_3_8,
meanSBP_PNC4, M06_BP_SYS_VSORRES_1_9, M06_BP_SYS_VSORRES_2_9, M06_BP_SYS_VSORRES_3_9,
meanDBP_PNC4, M06_BP_DIA_VSORRES_1_9, M06_BP_DIA_VSORRES_2_9, M06_BP_DIA_VSORRES_3_9,
meanSBP_PNC6, M06_BP_SYS_VSORRES_1_10, M06_BP_SYS_VSORRES_2_10, M06_BP_SYS_VSORRES_3_10,
meanDBP_PNC6, M06_BP_DIA_VSORRES_1_10, M06_BP_DIA_VSORRES_2_10, M06_BP_DIA_VSORRES_3_10,
meanSBP_PNC26, M06_BP_SYS_VSORRES_1_11, M06_BP_SYS_VSORRES_2_11, M06_BP_SYS_VSORRES_3_11,
meanDBP_PNC26, M06_BP_DIA_VSORRES_1_11, M06_BP_DIA_VSORRES_2_11, M06_BP_DIA_VSORRES_3_11,
meanSBP_PNC52, M06_BP_SYS_VSORRES_1_12, M06_BP_SYS_VSORRES_2_12, M06_BP_SYS_VSORRES_3_12,
meanDBP_PNC52, M06_BP_DIA_VSORRES_1_12, M06_BP_DIA_VSORRES_2_12, M06_BP_DIA_VSORRES_3_12,
meanSBP_Hosp_PNC, M19_BP_SYS_VSORRES_14,
meanDBP_Hosp_PNC, M19_BP_DIA_VSORRES_14)


write.csv(export_df, paste0('data_out/', "preeclampsia" ,".csv"), row.names=FALSE)
```
