...
mnh_list <- readRDS(file = '../MNH_list_2023-09-15.RDS')
```
...
mnh08_constructed_df <- mnh08_df %>% 
  # VISIT 1
  mutate(M08_MAT_VISIT_FORM_COMPLETED_1 = case_when(!is.na(M08_MAT_VISIT_MNH08_1) ~ 1, 
                                                    TRUE ~ 0),
         ...
         M08_MAT_VISIT_FORM_COMPLETED_3 = case_when(!is.na(M08_MAT_VISIT_MNH08_3) ~ 1, 
                                                    TRUE ~ 0),
         M08_MAT_VISIT_wPERSON_COMPLETED_3 = case_when(M08_MAT_VISIT_MNH08_3 %in% c(1,2) ~ 1, # changed order
                                                       TRUE ~ 0),
         # VISIT 4
...
merged_df <- merged_df %>%
  ...
  mutate(ENROLL_ONTIME = (EDD_BOE - as.difftime(280, unit="days")) + as.difftime(139, unit="days"), #EDD-280 = conception date + and then estimate the right hand of the window. Gives a date
         ...
         ANC28_ONTIME = (EDD_BOE - as.difftime(280, unit="days")) + as.difftime(216, unit="days"), # if miss ANC 28, you miss the visit, there is no late window
  ...
  mutate(M04_ENROLL_OVERDUE = ifelse(UploadDate>ENROLL_ONTIME & is.na(M04_MAT_VISIT_FORM_COMPLETED_1), 1, 0),
         M08_ENROLL_OVERDUE = ifelse(UploadDate>ENROLL_ONTIME & is.na(M08_MAT_VISIT_FORM_COMPLETED_1), 1, 0),#TODO DO THIS FOR MNH08
         
          # if this window ANC20 is passed, then this var would be NA b/c they don't have to have ANC20 visit. 
         M04_ANC20_OVERDUE = ifelse(UploadDate>ANC20_ONTIME & is.na(M04_MAT_VISIT_FORM_COMPLETED_2) & GA_ENROLL_WKS <= 17, 1, 0),
         M08_ANC20_OVERDUE = ifelse(UploadDate>ANC20_ONTIME & is.na(M08_MAT_VISIT_FORM_COMPLETED_2) & GA_ENROLL_WKS <= 17, 1, 0),
         
         M04_ANC28_OVERDUE = ifelse(UploadDate>ANC28_ONTIME & is.na(M04_MAT_VISIT_FORM_COMPLETED_3), 1, 0), #IF 1 this will mean she is missing the form.
         M08_ANC28_OVERDUE = ifelse(UploadDate>ANC28_ONTIME & is.na(M08_MAT_VISIT_FORM_COMPLETED_3), 1, 0),
         
         M04_ANC32_OVERDUE = ifelse(UploadDate>ANC32_ONTIME & is.na(M04_MAT_VISIT_FORM_COMPLETED_4), 1, 0),
         M08_ANC32_OVERDUE = ifelse(UploadDate>ANC32_ONTIME & is.na(M08_MAT_VISIT_FORM_COMPLETED_4), 1, 0),
         
         M04_ANC36_OVERDUE = ifelse(UploadDate>ANC36_ONTIME & is.na(M04_MAT_VISIT_FORM_COMPLETED_5), 1, 0),
         M08_ANC36_OVERDUE = ifelse(UploadDate>ANC36_ONTIME & is.na(M08_MAT_VISIT_FORM_COMPLETED_5), 1, 0)) %>% 
...
```{r}
write_rds(merged_df, '../data_out/merged_df.RDS')
```
...
#temp.df <- merged_df %>%
  # select(MOMID, SITE, DELIVERED_FF, M08_CBC_HB_LBORRES_1, M06_HB_POC_LBORRES_1) %>% filter(SITE=='Ghana') %>% filter(DELIVERED_FF==1)
...
anemia_df <- merged_df %>% 
  select(MOMID, PREGID, SITE, M09_DELIV_DSSTDAT_INF1_6, DELIVERED_FF, starts_with(c("M08_CBC_HB_LBORRES_", "M06_HB_POC_LBORRES_", "MAT_ANEMIA_MEAS")))
...
#table(anemia_df2$MAT_ANEMIA_MEAS_ANY, SITE = anemia_df2$SITE, useNA = "always")
# round(prop.table(table(anemia_df2$MAT_ANEMIA_MEAS_ANY, SITE = anemia_df2$SITE, useNA = "always"), margin=2)*100, 2)
```
...
############################
#### WRITE OUT CSV FOR HTN
###########################
```{r}
htn_df <- merged_df %>% 
  select(MOMID, PREGID, SITE, DELIVERED_FF, M04_ANC_OBSSTDAT_1, # Date ANC1 from MNH04
         M04_HPD_MHTERM_2, # Chronic hypertension or gHTN or PE or Don't know.  
         ...
         starts_with(c("M06_BP_SYS_VSORRES_", "M06_BP_DIA_VSORRES_", "M04_HTN_CMTRT_", 
                       "M04_HPD_OECTRT_", 
                       "M09_HDP_HTN_PROCCUR_", # L&D Medication
                       # ANC HOSP _13
                       "M19_HDP_HTN_MHOCCUR_",
                       # PROTEINURIA
                       "M08_UA_PROT_LBORRES",
...
#####################
merged_df <- merged_df %>%
  mutate(MAT_HEMOLYSIS = case_when((MAT_ANEMIA_MEAS4 == "severe" & M08_IBILIRUBIN_LBORRES_4 >=20.52 | M08_GAMMAGT_LBORRES_2) ~ 1,
                                    TRUE ~ as.numeric(0)))
