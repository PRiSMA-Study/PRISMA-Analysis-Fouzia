...
**Denominator:** Enrolled women who have an ANC-28 (VISIT 3).
<br>
<br>
...
diabetes_table <- df %>%
  ...
  summarise(
    ...
    "Missing Enrollment HbA1c: M08_HBA1C_PRCNT_1, n (%)" =
      paste0(sum(is.na(M08_HBA1C_PRCNT_1)),
             " (",
             format(round(sum(is.na(M08_HBA1C_PRCNT_1))/n()*100, 2),nsmall=2, digits=3),
             ")"),
...
diabetes_table  %>% 
  ...
  pack_rows("Gestational Diabetes (Measured) Category, n (%)", 5, 7, label_row_css = "color: steelblue;") %>% # WHAT DOES 5,6 HERE SAY? 
  pack_rows("Overt Diabetes (HbA1c)  Category, n (%)", 8, 10, label_row_css = "color: steelblue;") %>%
  pack_rows("Fasting Glucose Category, n (%)", 11, 13, label_row_css = "color: steelblue;") %>%
  pack_rows("OGTT 1-hr Category, n (%)", 14, 16, label_row_css = "color: steelblue;") %>%
  pack_rows("OGTT 2-hr Category, n (%)", 17, 19, label_row_css = "color: steelblue;")
