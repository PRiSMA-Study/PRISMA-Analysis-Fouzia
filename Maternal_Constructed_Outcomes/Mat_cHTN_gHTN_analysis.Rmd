...
```{r}
htn_df2 <- left_join(htn_df, window_subset_df, 
...
# CHRONIC HYPERTENSION MEASURED + TREATMENT + DIAG
# change this to just traetment or measured - take out Diagnosed. 
```{r}
htn_df <- htn_df %>% 
  ...
  mutate(CHRON_HYPER = case_when((CHRON_HYPER_MEAS ==1 | CHRON_HYPER_TREAT==1 | 
                                            ...
                                            (is.na(CHRON_HYPER_MEAS) & is.na(CHRON_HYPER_TREAT) & 
                                               is.na(CHRON_HYPER_DIAG)) ~ as.numeric(NA),
...
htn_df <- htn_df %>%
  ...
  mutate(CHRON_HYPER_MEAS_ANY2 = ifelse(is_chtn(meanSBP_ANC_LT20, meanDBP_ANC_LT20) & 
                                          any(is_chtn(meanSBP_ANC20, meanDBP_ANC20),
                                              ...
                                              is_chtn(meanSBP_Hosp_ANC, meanDBP_Hosp_ANC)), 1, 0),
...
#TODO ask about all(is.na) issue
```{r}
htn_df <- htn_df %>%
  rowwise() %>%
  mutate(CHRON_HYPER_MEAS_ANY1 = ifelse(any(is_chtn(meanSBP_ANC_LT20, meanDBP_ANC_LT20)), 1, 0),
         # OVERWRITES 0 WITH NA when all these vars are NA
...
temp.df <- htn_df %>% select(MOMID, SITE, CHRON_HYPER_MEAS_ANY1, M06_BP_SYS_VSORRES_1_1, M06_BP_SYS_VSORRES_2_1, M06_BP_SYS_VSORRES_3_1,
                             M06_BP_DIA_VSORRES_1_1, M06_BP_DIA_VSORRES_2_1, M06_BP_DIA_VSORRES_3_1, meanSBP_ANC_LT20, meanDBP_ANC_LT20) %>%
  filter(SITE=="Pakistan")
```
...
htn_df <- htn_df %>%
  ...
  mutate(GEST_HYPER_MEAS_ANY2 = ifelse(((sum(is_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                    ...
                                    CHRON_HYPER==0), 1, 0),
         # OVERWRITES 0 WITH NA when all these vars are NA
         GEST_HYPER_MEAS_ANY2 = ifelse(all(is.na(c(meanSBP_ANC20, meanDBP_ANC20, meanSBP_ANC28, meanDBP_ANC28, 
                                                   meanSBP_ANC32, meanDBP_ANC32, meanSBP_ANC36, meanDBP_ANC36,
                                                   meanSBP_IPC, meanDBP_IPC, meanSBP_Hosp_ANC, meanDBP_Hosp_ANC))), 
                                       as.numeric(NA),
                                       # OTHERWISE, IT LEAVES IT ALONE.
                                       GEST_HYPER_MEAS_ANY2))

table(htn_df$GEST_HYPER_MEAS_ANY2, htn_df$SITE, useNA = "always")
round(prop.table(table(htn_df$GEST_HYPER_MEAS_ANY2, htn_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
htn_df <- htn_df %>%
  ...
  mutate(GEST_HYPER_MEAS_ANY1 = ifelse(((sum(is_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                          ...
                                          CHRON_HYPER==0), 1, 0),
         # OVERWRITES 0 WITH NA when all these vars are NA
         GEST_HYPER_MEAS_ANY1 = ifelse(all(is.na(c(meanSBP_ANC20, meanDBP_ANC20, 
                                                   meanSBP_ANC28, meanDBP_ANC28, 
                                                   meanSBP_ANC32, meanDBP_ANC32, 
                                                   meanSBP_ANC36, meanDBP_ANC36,
                                                   meanSBP_IPC, meanDBP_IPC, 
                                       ...
                                       as.numeric(NA),
                                       # OTHERWISE, IT LEAVES IT ALONE.
                                       GEST_HYPER_MEAS_ANY1))

table(htn_df$GEST_HYPER_MEAS_ANY1, htn_df$SITE, useNA = "always")
round(prop.table(table(htn_df$GEST_HYPER_MEAS_ANY1, htn_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
table(GEST_HYPER_TREAT = htn_df$GEST_HYPER_TREAT, htn_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_TREAT = htn_df$GEST_HYPER_TREAT, SITE = htn_df$SITE, useNA = "always"), margin=2)*100,2)  
...
table(GEST_HYPER_MEAS1_TREAT = htn_df$GEST_HYPER_MEAS1_TREAT, htn_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_MEAS1_TREAT = htn_df$GEST_HYPER_MEAS1_TREAT, SITE = htn_df$SITE, useNA = "always"), margin=2)*100,2) 
...
htn_df <- htn_df %>% 
  mutate(SEVERE_GEST_HYPER_MEAS = ifelse(((sum(is_severe_ghtn(meanSBP_ANC20, meanDBP_ANC20),
                                            ...
                                            CHRON_HYPER==0), 1, 0),
         # OVERWRITES 0 WITH NA when all these vars are NA:
         SEVERE_GEST_HYPER_MEAS = ifelse(all(is.na(c(meanSBP_ANC20, meanDBP_ANC20, meanSBP_ANC28, meanDBP_ANC28,
                                                     meanSBP_ANC32, meanDBP_ANC32, meanSBP_ANC36, meanDBP_ANC36,
                                                     meanSBP_IPC, meanDBP_IPC, meanSBP_Hosp_ANC, meanDBP_Hosp_ANC))),
                                         as.numeric(NA),
                                         # OTHERWISE, IT LEAVES IT ALONE.
                                         SEVERE_GEST_HYPER_MEAS))
           
table(SEVERE_GEST_HYPER_MEAS = htn_df$SEVERE_GEST_HYPER_MEAS, htn_df$SITE, useNA = "always")
round(prop.table(table(SEVERE_GEST_HYPER_MEAS = htn_df$SEVERE_GEST_HYPER_MEAS, SITE = htn_df$SITE, useNA = "always"), margin=2)*100,2)           
           
# temp.df <- htn_df %>% select(MOMID, PREGID, GEST_HYPER_MEAS, starts_with("M04_HTN_CMTRT_")) # I DON'T NEED THIS CHRONIC HTN MEDICATION TREATMENT DATA. 
```

# GESTAITONAL HYPERTENSION MEASURED 2+ OR (1 MEASURED & TREATMENT) OR SEVERE gHTN (FINAL)
```{r}
...
# GESTATIONAL HYPERTENSION MEASURED 2+ OR (1 MEASURED & TREATMENT) OR SEVERE gHTN - "WITHOUT" PROTEINURIA (FINAL)
PROTEINURIA_ANC_IPC
```{r}
htn_df <- htn_df %>%
  mutate(GEST_HYPER_WO_PROTEINURIA = case_when(GEST_HYPER==1 & (PROTEINURIA_ANC_IPC==0 | is.na(PROTEINURIA_ANC_IPC)) ~ 1,
                                               all(is.na(c(GEST_HYPER, PROTEINURIA_ANC_IPC))) ~ as.numeric(NA),
                                               TRUE ~ as.numeric(0)))

table(GEST_HYPER_WO_PROTEINURIA = htn_df$GEST_HYPER_WO_PROTEINURIA, htn_df$SITE, useNA = "always")
round(prop.table(table(GEST_HYPER_WO_PROTEINURIA = htn_df$GEST_HYPER_WO_PROTEINURIA, SITE = htn_df$SITE, useNA = "always"), margin=2)*100,2)
```
...
htn_df <- htn_df %>%
  mutate(MAT_HELLP = case_when((MAT_HEMOLYSIS ==1 | M08_AST_UL_LBORRES_4 >= 90 | M08_ALT_UL_LBORRES_4 >= 120 | # AST AND ALT 2X THE UPPER NORMAL RANGE
                               ...
                               ((M19_CBC_PLATELET_LBORRES_13>=0 & M19_CBC_PLATELET_LBORRES_13 < 100) |
                                 M19_CREAT_LBORRESU_13 >97.26)) ~ 1,  # Creatinine is umol/L
...
table(MAT_PE_CHRONIC_HTN_MEAS = htn_df$MAT_PE_CHRONIC_HTN_MEAS, SITE = htn_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_CHRONIC_HTN_MEAS = htn_df$MAT_PE_CHRONIC_HTN_MEAS, SITE = htn_df$SITE, useNA = "always"), margin=2)*100, 2)
```
...
table(MAT_PE_GEST_HTN_MEAS = htn_df$MAT_PE_GEST_HTN_MEAS, SITE = htn_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_GEST_HTN_MEAS = htn_df$MAT_PE_GEST_HTN_MEAS, SITE = htn_df$SITE, useNA = "always"), margin=2)*100, 2)
...
table(MAT_PE_ANY_MEAS = htn_df$MAT_PE_ANY_MEAS, SITE = htn_df$SITE, useNA = "always")
round(prop.table(table(MAT_PE_ANY_MEAS = htn_df$MAT_PE_ANY_MEAS, SITE = htn_df$SITE, useNA = "always"), margin=2)*100, 2)

```
# WRITE CSV OUT FOR HTML
```{r}

write.csv(htn_df, '../data_out/mat_htn_tables.csv')
