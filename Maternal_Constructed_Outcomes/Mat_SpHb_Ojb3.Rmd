...
UploadDate =  "2024-01-26" # "2024-02-02"
...
# I devidied by 1000 b/c it is then 
df <- df %>% 
...
print(variance_MOMID)
print(variance_TYPE_VISIT)
print(variance_SITE)
print(residual_variance)

# ICC:
ICC_calc <- (variance_MOMID + variance_TYPE_VISIT + variance_SITE)/(variance_MOMID + variance_TYPE_VISIT + variance_SITE+ residual_variance)
ICC_calc
# with 01/26/2024 data: ICC = 0.383
...
#fit glmm says Qing.  Specify a link function. family = binomial. No link. 
O2_glmm_model_cbc <- lmer(CBC_anemia ~ M08_CBC_HB_LBORRES + (1 | MOMID) + (1 | SITE) + (1 | TYPE_VISIT),
                    control = ctrl,
                    
                    data = df2)

O2_glmm_model_sphb <- lmer(SPHB_anemia ~ M06_SPHB_LBORRES + (1 | MOMID) + (1 | SITE) + (1 | TYPE_VISIT),
                    control = ctrl,
                    data = df2)

O2_glmm_model_cbc <- lmer(M08_CBC_HB_LBORRES ~  (1 | MOMID) + (1 | SITE) + (1 | TYPE_VISIT),
                    control = ctrl,
                    data = df2)

# O2_glmm_model <- glmer(cbind(CBC_anemia, SPHB_anemia) ~ 1 + (1 | MOMID) + (1 | SITE) + (1 | TYPE_VISIT),
#                     family = binomial(link = "logit"),
#                     data = df2)

# Predict measurements from both devices# Predict measurements from both devicesTYPE_VISIT
df2$predicted_CBC_anemia <- predict(O2_glmm_model_cbc, newdata = df2, type = "response")
df2$predicted_SPHB_anemia <- predict(O2_glmm_model_sphb, newdata = df2, type = "response")

# Convert predicted probabilities to binary outcomes (diseased/non-diseased)
df2$predicted_CBC_anemia_binary <- ifelse(df2$predicted_CBC_anemia >= 0.5, 1, 0)
df2$predicted_SPHB_anemia_binary <-ifelse(df2$predicted_SPHB_anemia >= 0.5, 1, 0)


temp.df <- df2 %>% 
  select(SITE, MOMID, TYPE_VISIT, predicted_CBC_anemia_binary, predicted_SPHB_anemia_binary )

# Check expected structure
str(df2$predicted_CBC_anemia_binary)
str(df2$predicted_SPHB_anemia_binary)

# Check for NA values in the binary outcome 
if (any(is.na(df2$predicted_CBC_anemia_binary)) || any(is.na(df2$predicted_SPHB_anemia_binary))) {
  stop("NA values detected in the binary outcome vectors")
}

# Print lengths of the vectors
cat("Length of predicted_CBC_anemia_binary:", length(df2$predicted_CBC_anemia_binary), "\n")
cat("Length of predicted_SPHB_anemia_binary:", length(df2$predicted_SPHB_anemia_binary), "\n")

# Print unique values in each vector
cat("Unique values in predicted_CBC_anemia_binary:", unique(df2$predicted_CBC_anemia_binary), "\n")
cat("Unique values in predicted_SPHB_anemia_binary:", unique(df2$predicted_SPHB_anemia_binary), "\n")

# Create  a contingency table.
# table_result <- table(df2$predicted_CBC_anemia_binary, df2$predicted_SPHB_anemia_binary)

# Compute Cohen's kappa
kappa_result <- kappa2(df2[, c( "predicted_SPHB_anemia_binary", "predicted_CBC_anemia_binary")]) # I have to use this structure of [], otherwise it doesn't work. Otherwise, I have to have a single matrix or dataframe.  

# Print the result
print(kappa_result) # OK to have negative kappa. 
```
