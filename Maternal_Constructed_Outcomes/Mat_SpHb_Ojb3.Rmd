...
```{r}
merged_df3 <- merged_df2 %>% filter(TYPE_VISIT %in% c(1,2,3,5,6))
temp_df <- merged_df3 %>% select(SITE, MOMID, TYPE_VISIT, M06_SPHB_LBORRES, M08_CBC_HB_LBORRES)
```
...
mixed_model <- lmer(diff_hb ~ TYPE_VISIT + (1 | MOMID) + (1 | SITE), data = merged_df3)
