...
#TODO SINCE ZAMBIA DATA HAS NO LMP OR GA, ALL THE OTHER VARS DON'T GET CREATED IN MY WINDOW_SUBSET_DF, SO I COULD CREATE A SEP SUBSET OF INDIVIDUALS WHO DON'T HAVE MNH08 MISSING AND THEN MERGE THAT ONTO (ROWBIND()) OTHER COUNTRIES - WILL JUST TO MAKE A SPECIAL NOTE THAT ZAMBIA DATA IS NOT BASED ON GA>20
...
# zambia_diabetes_df3 <- diabetes_df2 %>% filter(SITE=="Zambia")
# diabetes_df4 <- rbind(diabetes_df3, zambia_diabetes_df3) # stack the zambia data here.

temp.df2 <- merged_df %>% filter(SITE=="Zambia") %>%
  select(MOMID, SITE, M08_GA_AT_VISIT_3, M01_US_GA_WKS_AGE_FTS1_1, M08_LBSTDAT_3, GA_US_DAYS_FTS1_1, LMP_GA_DAYS) # FOR ZAMBIA I SHOULD JUST TAKE !is.na(M08_LBSTDAT_3 b/c that info is there.)
```
-5 = missing
-7 = missing
If i am using ANC28_PASS variable (has the ANC28 window passed; if yes = she should have a form), i need to use M08_OVERDUE variable with it to see if she is overdue for the form that she should have or not. (if OVERDUE==1, then that means she's missing the form and the variable will be missing also). Otherwise, I can also just use MAT_VISIT_FORM variable alone. 

Use: ANC28_PASS==1 and M08_OVERDUE together 
M08_OVERDUE - will show that MNH08 form is missing.They get an =1 only if they are overdue for their visit, so if 0 or NA, she is not overdue. 
otherwise: 
...
diabetes_df <- diabetes_df %>% filter(!is.na(M08_LBSTDAT_3)) %>%
  mutate(OVERT_DIA = case_when(M08_HBA1C_PRCNT_1>=6.5 ~ 1,
                               is.na(M08_HBA1C_PRCNT_1) ~ as.numeric(NA),
         TRUE ~ as.numeric(0)))
...
diabetes_df <- diabetes_df %>% filter(!is.na(M08_LBSTDAT_3)) %>%
  mutate(OGTT_PRE = case_when(M08_BGLUC_PRETEST_MMOLL_LBORRES_3>=5.1 ~ 1,
                              (is.na(M08_BGLUC_PRETEST_MMOLL_LBORRES_3)) ~ as.numeric(NA),
         TRUE ~ as.numeric(0)))
...
diabetes_df <- diabetes_df %>% filter(!is.na(M08_LBSTDAT_3)) %>%
  mutate(OGTT_1hr = case_when(M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_3>=10.0 ~ 1,
                              (is.na(M08_BGLUC_ORAL_1HR_MMOLL_LBORRES_3)) ~ as.numeric(NA),
         TRUE ~ as.numeric(0))) #%>%
...
write.csv(diabetes_df, '../data_out/Mat_diabetes_VISIT3.csv') # restricted to women who have a visit 3. 
